  ...
  try {
    ...
    if (!stripeCustomerId || stripeCustomerId.startsWith('local_')) {
      ...
      try {
        ...
        if (process.env.STRIPE_INVOICE_TEMPLATE_ID) {
          console.warn(`[Checkout Subscription] Template facture invalide (${process.env.STRIPE_INVOICE_TEMPLATE_ID}), création client sans template:`, templateError.message);
