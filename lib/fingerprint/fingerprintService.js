...
function extractIp(request) {
  ...
  // Debug temporaire pour diagnostiquer les headers IP
  console.log(`[Fingerprint] Headers IP: cf-connecting-ip=${cfIp || 'absent'}, x-real-ip=${realIp || 'absent'}, x-forwarded-for=${forwardedFor || 'absent'}`);
...
export async function checkAndSaveFingerprint(userId, rawVisitorId, request) {
  ...
  if (existingFingerprint) {
    ...
    // Vérifier si au moins un compte matché a vérifié son email
    // Si tous les comptes matchés sont non vérifiés, on accorde quand même les crédits de bienvenue
    const allMatches = await prisma.browserFingerprint.findMany({
      where: { OR: conditions },
      select: { userId: true },
      distinct: ['userId'],
    });

    const matchedUserIds = allMatches.map(f => f.userId).filter(Boolean);
    const hasDeletedAccounts = allMatches.some(f => f.userId === null);

    // Comptes supprimés → conservateur, on considère comme vérifiés
    let hasVerifiedAccount = hasDeletedAccounts;
    if (!hasVerifiedAccount && matchedUserIds.length > 0) {
      const verifiedCount = await prisma.user.count({
        where: { id: { in: matchedUserIds }, emailVerified: { not: null } },
      });
      hasVerifiedAccount = verifiedCount > 0;
    }

    console.log(`[Fingerprint] Comptes matchés: ${matchedUserIds.length} existants, ${hasDeletedAccounts ? 'avec' : 'sans'} comptes supprimés, hasVerifiedAccount=${hasVerifiedAccount}`);
    return { isDuplicate: true, matchedUserId: existingFingerprint.userId, matchType, hasVerifiedAccount };
  }
