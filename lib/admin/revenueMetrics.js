...
export async function getCreditPacksRevenue() {
  ...
  purchases.forEach((purchase) => {
    ...
    // Déterminer le prix réellement payé (après réductions/coupons)
    let actualPrice = pack.price; // Fallback : prix catalogue
    let hasPricePaid = false;

    // Vérifier le metadata pour pricePaid (nouvelles transactions)
    if (purchase.metadata) {
      try {
        const meta = JSON.parse(purchase.metadata);
        if (meta.pricePaid !== undefined && meta.pricePaid !== null) {
          actualPrice = meta.pricePaid;
          hasPricePaid = true;
        }
      } catch (e) {
        // Metadata JSON invalide, utiliser le fallback
      }
    }

    // Fallback pour anciennes transactions : pas de pricePaid + pas de paymentIntent = checkout gratuit (coupon 100%)
    if (!hasPricePaid && !purchase.stripePaymentIntentId) {
      actualPrice = 0;
    }

    packSales[pack.id].salesCount++;
    packSales[pack.id].totalRevenue += actualPrice;
