 ...
 * la suppression de l'utilisateur supprimera automatiquement toutes les
 * données liées (cvFiles, accounts, feedbacks, etc.)
 *
...
export async function deleteUserCompletely(userId) {
  try {
    ...
    // 3. Supprimer l'utilisateur de la DB
...
export async function createManualUser({ email, name, password, role = 'USER' }) {
  try {
    ...
    if (!passwordValidation.valid) {
      return { success: false, error: passwordValidation.errors?.[0] || 'Mot de passe trop faible' };
    }
    ...
    const user = await prisma.user.create({
      data: {
        ...
        onboardingState: DEFAULT_ONBOARDING_STATE, // Cohérent avec l'inscription normale
      },
      select: {
        id: true,
        email: true,
