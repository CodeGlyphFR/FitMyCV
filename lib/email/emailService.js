...
import {
  ...
  getInactivityWarningEmailTemplate,
...
export async function createPasswordResetToken(email) {
  ...
  // Stocker le hash du token (pas le token brut) pour protéger contre les dumps DB
...
export async function verifyPasswordResetToken(token) {
  ...
  // Comparer le hash du token fourni avec celui stocké en DB
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
...
export const sendPurchaseCreditsEmail = createEmailSender('purchase_credits', {
  ...
  bcc: () => process.env.TRUSTPILOT_INVITE_EMAIL || null,
});

/**
 * Envoie un email d'avertissement d'inactivité (30 jours avant suppression)
 */
export const sendInactivityWarningEmail = createEmailSender('inactivity_warning', {
  templateName: 'inactivity_warning',
  defaultSubject: 'Votre compte FitMyCV.io sera supprimé dans 30 jours',
  buildVariables: ({ name, lastActivity }, siteUrl) => {
    const lastActivityDate = new Date(lastActivity).toLocaleDateString('fr-FR', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    });
  ...
  getFallbackHtml: getInactivityWarningEmailTemplate,
});
