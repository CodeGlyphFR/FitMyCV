...
async function renderMailyTemplate(designJson) {
  ...
  // Simple approach: just replace white backgrounds with the chosen backgroundColor
  // Don't do complex transformations that might break the HTML
 ...
 * @param {string|Object|null} designJson - The Maily/TipTap JSON content
 * @returns {boolean} true if designJson contains actual content
 */
...
/**
 * Check if a color is "dark" (luminosity < 50%)
 * @param {string} hexColor - Hex color like "#020617" or "#fff"
 ...
 * @param {string} params.provider - Provider utilise ('smtp' ou 'resend')
 * @param {string} params.providerId - ID du message du provider (optionnel)
 */
...
export async function sendVerificationEmail({ email, name, token, userId }) {
  ...
  if (template && isValidDesignJson(template.designJson)) {
    // Priorité au designJson (rendu via renderMailyTemplate avec format table-based)
  ...
  console.log('[emailService] Tentative d\'envoi d\'email de vérification:', {
    ...
    usingDesignJson: !!(template?.designJson),
    triggerName: 'email_verification',
    smtpConfigured: isSmtpConfigured(),
    resendConfigured: isResendConfigured(),
  });

  // Envoi via la couche de transport (gère SMTP/Resend et fallback)
  ...
  if (!result.success) {
    console.error('[emailService] Échec envoi email de vérification:', {
      error: result.error,
      provider: result.provider,
...
export async function sendPasswordResetEmail({ email, name, token, userId }) {
  ...
  if (!result.success) {
    console.error('[emailService] Échec envoi email de réinitialisation:', {
      error: result.error,
      provider: result.provider,
      usedFallback: result.usedFallback,
    });
...
export async function sendEmailChangeVerification({ email, name, token, userId }) {
  ...
  if (!result.success) {
    console.error('[emailService] Échec envoi email de changement d\'adresse:', {
      error: result.error,
      provider: result.provider,
      usedFallback: result.usedFallback,
    });
...
export async function sendWelcomeEmail({ email, name, userId }) {
  ...
  let html, subject;
  const variables = { userName: name, loginUrl, welcomeCredits: String(welcomeCredits) };
  ...
  if (template && isValidDesignJson(template.designJson)) {
    ...
    // Fallback vers htmlContent si pas de designJson valide
    html = substituteVariables(template.htmlContent, variables);
    subject = substituteVariables(template.subject, { userName: name });
  } else {
    // Fallback to hardcoded template
  ...
  if (!result.success) {
    console.error('[emailService] Échec envoi email welcome:', {
      error: result.error,
      provider: result.provider,
      usedFallback: result.usedFallback,
    });
...
export async function sendPurchaseCreditsEmail({ email, name, userId, creditsAmount, totalPrice, invoiceUrl }) {
  ...
  if (!result.success) {
    console.error('[emailService] Échec envoi email purchase_credits:', {
      error: result.error,
      provider: result.provider,
      usedFallback: result.usedFallback,
    });
