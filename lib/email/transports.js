 ...
 */
async function sendViaSMTP({ to, subject, html, bcc }) {
...
export async function sendEmail({ to, subject, html, bcc }) {
  ...
  if (EMAIL_PROVIDER === 'smtp') {
    ...
    return sendViaSMTP({ to, subject, html, bcc });
  }
  ...
  if (EMAIL_PROVIDER === 'resend') {
    ...
    return sendViaResend({ to, subject, html, bcc });
  }

  // Mode auto: SMTP principal, Resend fallback
  if (smtpAvailable) {
    console.log('[Email] Tentative envoi via SMTP...');
    const smtpResult = await sendViaSMTP({ to, subject, html, bcc });
    ...
    if (resendAvailable) {
      ...
      const resendResult = await sendViaResend({ to, subject, html, bcc });
      return {
        ...resendResult,
        usedFallback: true,
