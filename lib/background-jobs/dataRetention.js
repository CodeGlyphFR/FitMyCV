  ...
  try {
    ...
    for (const user of inactiveUsers) {
      ...
      try {
        ...
        notifiedCount++;
        console.log(`${LOG_PREFIX} Email d'avertissement envoyé à ${user.email}`);
      } catch (emailError) {
        console.error(`${LOG_PREFIX} Erreur envoi email à ${user.email}:`, emailError.message);
      }
    }
  ...
  try {
    ...
    for (const user of inactiveUsers) {
      try {
        ...
        if (result.success) {
          deletedCount++;
          console.log(`${LOG_PREFIX} Compte inactif supprimé: ${user.email}`);
        } else {
          console.error(`${LOG_PREFIX} Échec suppression ${user.email}:`, result.error);
        }
      } catch (deleteError) {
        console.error(`${LOG_PREFIX} Erreur suppression ${user.email}:`, deleteError.message);
      }
    }
