...
import { ensureUserCvDir, writeUserCvFile, deleteUserCvFile } from "@/lib/cv-core/storage";
...
async function getImportPdfCv() {
  const module = await import("@/lib/features/import-pdf");
...
async function isFirstImport(userId) {
  try {
    ...
    return false; // En cas d'erreur, on considère que ce n'est pas un premier import
  }
}
...
export async function runImportPdfJob({
  ...
  // Vérifier si c'est le premier import de l'utilisateur
  ...
  try {
    ...
    // Retourne { content: string, language: string } (optimisé pour réduire tokens)
    const importPdfCv = await getImportPdfCv();
    const importResult = await importPdfCv({
      pdfFilePath,
      signal: abortController.signal, // Passer le signal pour annulation
      userId, // Pour la télémétrie OpenAI
      isFirstImport: isFirstPdfImport, // Passer le flag pour sélection du modèle
    });

    // Nouveau format de retour : { content, language }
    // La langue est detectee via Vision API dans importPdf.js (fiable)
    cvContent = importResult.content;
    detectedLanguage = importResult.language;
  ...
  try {
    ...
    // La langue est maintenant détectée lors de l'extraction (plus d'appel OpenAI supplémentaire)
    // Elle est déjà incluse dans cvContent via reconstructCv()

    // Sauvegarder/mettre à jour le CvFile en base avec la langue
    await prisma.cvFile.upsert({
      where: { userId_filename: { userId, filename } },
      update: {
        language: detectedLanguage, // Mettre à jour la langue si le fichier existe
      },
      create: {
        userId,
        filename,
        language: detectedLanguage, // Stocker la langue détectée
      },
    });
