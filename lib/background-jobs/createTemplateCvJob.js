...
async function getCreateTemplateCv() {
  const module = await import("@/lib/features/template-generation");
...
export async function runCreateTemplateCvJob({
  ...
  try {
    ...
    // Callback pour mettre à jour le titre dès que l'offre est extraite
  ...
  for (let i = 0; i < generatedContents.length; i++) {
    ...
    try {
      ...
      if (detectedLanguage) {
        console.log(`[createTemplateCvJob] Langue de l'offre utilisée: ${detectedLanguage}`);
      } else {
