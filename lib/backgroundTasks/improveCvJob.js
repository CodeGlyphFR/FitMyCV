...
function applySummaryDiff(originalSummary, modifications) {
  ...
  // Appliquer les modifications aux domains
  if (modifications.domains) {
    modifiedSummary.domains = applyArrayDiff(
      modifiedSummary.domains || [],
      modifications.domains
    );
  }

  // Appliquer les modifications aux key_strengths
  if (modifications.key_strengths) {
    modifiedSummary.key_strengths = applyArrayDiff(
      modifiedSummary.key_strengths || [],
      modifications.key_strengths
    );
  }
...
async function runPipelineV2({
  ...
  if (!preprocessResult.success || !preprocessResult.data) {
    ...
    const { experienceImprovements, projectImprovements, newProjects } = preprocessResult.data;
    ...
    const languageTask = hasLanguages ? limit(async () => {
      try {
        if (abortController.signal.aborted) throw new Error('Task cancelled');
        ...
        const result = await improveLanguages({
          ...
          suggestion: null, // Pas de suggestion spécifique, optimisation auto
          jobOffer,
          cvLanguage,
          signal: abortController.signal,
          userId,
