...
export async function runImproveCvJob({
  ...
  try {
    ...
    console.log(`[improveCvJob] Amélioration terminée - Score estimé: ${result.newScoreEstimate}/100`);
  ...
  try {
    ...
    if (replaceExisting) {
      ...
      await prisma.cvFile.update({
        where: {
          userId_filename: { userId, filename: improvedFilename }
        },
        data: {
    ...
    // pour forcer l'utilisateur à recalculer le score
    if (replaceExisting) {
      await prisma.cvFile.update({
        where: {
          userId_filename: { userId, filename: improvedFilename }
      ...
      console.log(`[improveCvJob] ✅ Données d'analyse supprimées pour forcer recalcul du score`);
    }
