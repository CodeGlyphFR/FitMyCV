...
export async function runImproveCvJob({
  ...
  // Calculer les différences PROGRAMMATIQUES entre le CV amélioré et l'original
  // Ceci permet d'avoir des diffs item-level (skills ajoutés/supprimés, bullets modifiés, etc.)
  // au lieu des descriptions haut niveau de l'IA
  ...
  // Utiliser les diffs programmatiques si disponibles, sinon fallback sur l'IA
  const changesMade = programmaticChanges.length > 0
    ? programmaticChanges
  ...
  try {
    // Variable pour stocker la version créée (pour initializeReviewState)
    let createdVersion = null;

    // Si on remplace le CV existant, créer une version de sauvegarde AVANT modification
    if (replaceExisting) {
      try {
        createdVersion = await createCvVersionWithTracking(
          userId,
          cvFile,
          'Avant optimisation IA',
          'optimization'
        );
    ...
    // Récupérer le cvRecord pour avoir jobOfferId
    ...
    if (replaceExisting) {
      ...
      await updateCvFile(userId, improvedFilename, {
        ...
        optimiseUpdatedAt: new Date(),
        updatedAt: new Date(),
        // Conserver le score avant optimisation pour affichage
    ...
    // Initialiser l'état de review pour permettre accept/reject des modifications
    if (replaceExisting && createdVersion !== null && changesMade.length > 0) {
      try {
        await initializeReviewState(
          userId,
          improvedFilename,
          changesMade,
          createdVersion - 1 // La version précédente (celle qu'on a sauvegardée)
        );
