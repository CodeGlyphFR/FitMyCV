...
export async function runCalculateMatchScoreJob({
  ...
  try {
    ...
    // ❌ En cas d'erreur, INCRÉMENTER le compteur pour restituer le token (car il a été décrémenté au début)
    if (!isAutomatic) {
      ...
      if (userRecord && userRecord.matchScoreRefreshCount < TOKEN_LIMIT) {
        ...
        console.log(`[calculateMatchScoreJob] ❌ Erreur - Token restitué: ${userRecord.matchScoreRefreshCount + 1}/${TOKEN_LIMIT}`);
      }
    }
  ...
  try {
    ...
    if (!isAutomatic) {
      ...
      console.log(`[calculateMatchScoreJob] ✅ Calcul réussi - Tokens restants: ${finalRefreshCount}/${TOKEN_LIMIT}`);
    } else {
    ...
    try {
      await trackMatchScore({
        ...
        isAutomatic,
        tokensUsed: isAutomatic ? 0 : 1,
        tokensRemaining: finalRefreshCount,
        duration,
        status: 'success',
      });
    } catch (trackError) {
