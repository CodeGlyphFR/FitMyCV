...
export async function runGenerateCvJobOptimized({
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      // Créer/Mettre à jour l'entrée CvFile
      ...
      console.log(`[generateCvJobOptimized] ✅ CV généré : ${filename}`);
    } catch (error) {
  ...
  console.log(`[generateCvJobOptimized] ✨ Job terminé - ${createdFiles.length} CV générés`);
}