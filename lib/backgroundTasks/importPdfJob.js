...
export async function runImportPdfJob({
  ...
  try {
    ...
    // Détecter la langue du CV et l'ajouter au JSON
    try {
      ...
      console.log(`[importPdfJob] Langue détectée: ${detectedLanguage}`);
    } catch (langError) {
      console.error('[importPdfJob] Erreur détection langue (non-bloquant):', langError.message);
      // Non-bloquant - continuer sans langue définie
    }
