...
export async function runGenerateCvFromJobTitleJob({
  ...
  try {
    ...
    // Définir la langue du CV à partir du paramètre language (pas de détection OpenAI nécessaire)
    try {
      const cvData = JSON.parse(enriched);
      const { normalizeLanguageInput } = await import('@/lib/cv/languageConstants');
      cvData.language = normalizeLanguageInput(language);
      enriched = JSON.stringify(cvData, null, 2);
      console.log(`[generateCvFromJobTitleJob] Langue définie: ${cvData.language}`);
    } catch (langError) {
      console.error('[generateCvFromJobTitleJob] Erreur définition langue (non-bloquant):', langError.message);
    }
