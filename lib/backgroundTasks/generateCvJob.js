...
async function persistGeneratedFiles({ userId, workspaceDir, generatedFiles, sourceUrl, sourceUpload }) {
  const created = [];
  ...
  for (const file of generatedFiles) {
    ...
    try {
      ...
      if (sourceUrl) {
        try {
          await setCvSource(userId, file, 'link', sourceUrl);
        } catch (sourceError) {
          console.error(`Impossible d'enregistrer la source pour ${file}:`, sourceError);
        }
      } else if (sourceUpload) {
        try {
          await setCvSource(userId, file, 'pdf', sourceUpload);
        } catch (sourceError) {
          console.error(`Impossible d'enregistrer la source pour ${file}:`, sourceError);
        }
      }
...
export async function runGenerateCvJob({
  ...
  const cancellationInterval = setInterval(async () => {
    if (await shouldTerminate(taskId, userId)) {
      cancelled = true;
      console.log(`[generateCvJob] Annulation de la tâche ${taskId}`);
      clearInterval(cancellationInterval);
      pythonProcess.kill('SIGTERM');
      setTimeout(() => {
        if (!pythonProcess.killed) {
          console.log(`[generateCvJob] Force kill de la tâche ${taskId}`);
  ...
  const exitCode = await new Promise(resolve => {
    ...
    pythonProcess.on("close", (code, signal) => {
      console.log(`[generateCvJob] Tâche ${taskId} terminée - code: ${code}, signal: ${signal}, cancelled: ${cancelled}`);
      off();
      // Si le signal est SIGTERM ou SIGKILL et que nous n'avons pas d'exit code valide, c'est une annulation
      if (cancelled || (signal && (signal === 'SIGTERM' || signal === 'SIGKILL') && code === null)) {
        resolve(-999);
      } else {
        resolve(code);
      }
  ...
  if (exitCode === 0) {
    const sourceUrl = Array.isArray(payload.links) && payload.links.length > 0 ? payload.links[0] : null;
