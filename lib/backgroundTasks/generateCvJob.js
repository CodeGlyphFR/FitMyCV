...
import { computeCvDiff, initializeReviewState } from "@/lib/cv/changeTracking";
...
export async function runGenerateCvJob({
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      if (detectedLanguage) {
        console.log(`[generateCvJob] Langue de l'offre utilisée: ${detectedLanguage}`);
      } else {
        console.log(`[generateCvJob] Langue de l'offre non disponible`);
      }
      ...
      const cvFileRecord = await prisma.cvFile.upsert({
      ...
      // Stocker la version de référence (version 0) pour permettre la comparaison future
      // Le CV de référence est stocké pour pouvoir afficher les différences
      if (referenceContent && cvFileRecord) {
        try {
          const referenceJson = typeof referenceContent === 'string'
          ...
          await prisma.cvVersion.upsert({
            ...
            update: {
              ...
              changelog: `Adapté depuis ${referenceFile}`,
          ...
          console.log(`[generateCvJob] Version de référence stockée pour ${filename}`);

          // Calculer les différences et initialiser l'état de review
          try {
            const currentCvJson = JSON.parse(result.cvContent);
            const changesMade = computeCvDiff(currentCvJson, referenceJson);

            // LOG pour debug
            console.log(`[generateCvJob] computeCvDiff a détecté ${changesMade.length} changement(s) pour ${filename}`);

            if (changesMade.length === 0) {
              // Log pour comprendre pourquoi aucun changement détecté
              console.log(`[generateCvJob] DEBUG - Reference summary:`, referenceJson?.summary?.description?.substring(0, 100));
              console.log(`[generateCvJob] DEBUG - Current summary:`, currentCvJson?.summary?.description?.substring(0, 100));
              console.log(`[generateCvJob] DEBUG - Reference skills count:`, referenceJson?.skills?.hard_skills?.length);
              console.log(`[generateCvJob] DEBUG - Current skills count:`, currentCvJson?.skills?.hard_skills?.length);
            }

            // TOUJOURS initialiser l'état de review pour les adaptations
            // Même si changesMade est vide, cela permet de voir la v0 et naviguer
            await initializeReviewState(userId, filename, changesMade, 0);
            console.log(`[generateCvJob] État de review initialisé pour ${filename}`);
          } catch (reviewError) {
            // Log l'erreur complète, pas juste le message
            console.error(`[generateCvJob] Erreur initialisation review:`, reviewError);
          }
        } catch (versionError) {
          console.warn(`[generateCvJob] Impossible de stocker la version de référence:`, versionError.message);
        }
      }
