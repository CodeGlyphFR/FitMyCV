 ...
 * @returns {Promise<{balance: number, totalPurchased: number, totalUsed: number, totalRefunded: number, totalGifted: number, balanceAfterLastPurchase: number}>}
 */
...
export async function debitCredit(userId, amount, type = 'usage', metadata = {}) {
  ...
  try {
    ...
    // Tracking PostHog — Crédits utilisés
    trackServerEvent(userId, 'credits_used', {
      amount,
      feature: metadata.featureName || null,
      type,
      remaining_balance: result.balance.balance,
    });

    // Émettre l'événement pour mise à jour temps réel via SSE
...
export async function refundCredit(userId, originalTransactionId, reason = 'Remboursement automatique') {
  try {
    // Vérifier d'abord si déjà remboursée (hors transaction pour éviter lock inutile)
    ...
    if (existingTransaction?.refunded) {
      console.log(`[Credits] Transaction ${originalTransactionId} déjà remboursée, ignoré`);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Double-check dans la transaction (race condition)
    ...
    // Gérer le cas déjà remboursé (race condition)
    if (result.alreadyRefunded) {
      console.log(`[Credits] Transaction ${originalTransactionId} déjà remboursée (race condition), ignoré`);
      return { success: true, alreadyRefunded: true };
    }
...
export async function grantCredits(userId, amount, type = 'purchase', metadata = {}) {
  ...
  // Vérifier que l'utilisateur existe avant de créer la transaction (évite FK violation)
  ...
  if (!userExists) {
    console.error(`[Credits] Utilisateur ${userId} introuvable - impossible d'attribuer ${amount} crédit(s) (type: ${type})`);
    return { success: false, error: `Utilisateur ${userId} introuvable` };
  }
  ...
  try {
    ...
    // Transaction atomique
    const result = await prisma.$transaction(async (tx) => {
      // Construire le JSON metadata avec les champs disponibles
      const metadataObj = {};
      if (metadata.source) metadataObj.source = metadata.source;
      if (metadata.pricePaid !== undefined) metadataObj.pricePaid = metadata.pricePaid;
      ...
      // Pour les achats, calculer la nouvelle balance totale pour balanceAfterLastPurchase
      ...
      if (type === 'purchase') {
        ...
        balanceAfterLastPurchaseValue = (currentBalance?.balance || 0) + amount;
      }
    ...
    // Tracking PostHog — Crédits achetés (uniquement les achats, pas les gifts)
    if (type === 'purchase') {
      trackServerEvent(userId, 'credits_purchased', {
        amount,
        price_paid: metadata.pricePaid ?? null,
    ...
    // Gérer la race condition: si unique constraint sur stripePaymentIntentId, c'est déjà traité
    if (error.code === 'P2002' && metadata.stripePaymentIntentId) {
      const target = error.meta?.target;
      if (target?.includes('stripePaymentIntentId') || target === 'stripePaymentIntentId') {
        console.log(`[Credits] Race condition détectée - transaction déjà créée pour paymentIntent ${metadata.stripePaymentIntentId}`);
...
/**
 * Vérifie si un utilisateur a déjà acheté des crédits
 * @param {string} userId - ID de l'utilisateur
 * @returns {Promise<boolean>}
 */
export async function hasPurchasedCredits(userId) {
  const balance = await getCreditBalance(userId);
  return balance.totalPurchased > 0;
}
