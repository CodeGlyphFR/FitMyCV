    ...
    for (const plan of plans) {
      try {
        ...
        if (!stripeProduct) {
          ...
          // Mettre à jour le nom et la description si ils ont changé
          const expectedDescription = plan.description || `Plan ${plan.name}`;
          if (stripeProduct.name !== plan.name || stripeProduct.description !== expectedDescription) {
    ...
    for (const pack of packs) {
      try {
        ...
        if (!stripeProduct) {
          ...
          if (stripeProduct.name !== pack.name || stripeProduct.description !== expectedDescription) {
            stripeProduct = await stripe.products.update(stripeProduct.id, {
              name: pack.name,
              description: expectedDescription,
              metadata: { packId: pack.id.toString(), creditAmount: pack.creditAmount.toString() },
