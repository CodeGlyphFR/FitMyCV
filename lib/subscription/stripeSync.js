      ...
      try {
        ...
        if (!stripeProduct) {
          stripeProduct = await stripe.products.create({
            ...
            tax_code: 'txcd_1.8.5.21', // SaaS - Services fournis par voie électronique
          ...
          if (stripeProduct.name !== plan.name || stripeProduct.description !== expectedDescription || stripeProduct.tax_code !== 'txcd_1.8.5.21') {
            stripeProduct = await stripe.products.update(stripeProduct.id, {
              name: plan.name,
              description: expectedDescription,
              tax_code: 'txcd_1.8.5.21',
              metadata: { planId: plan.id.toString() },
            });
    ...
    for (const pack of packs) {
      try {
        ...
        if (!stripeProduct) {
          ...
          if (stripeProduct.name !== pack.name || stripeProduct.description !== expectedDescription || stripeProduct.tax_code !== 'txcd_1.8.5.21') {
            stripeProduct = await stripe.products.update(stripeProduct.id, {
              name: pack.name,
              description: expectedDescription,
              tax_code: 'txcd_1.8.5.21',
