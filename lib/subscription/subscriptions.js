...
export async function getSubscriptionSummary(userId) {
  ...
  // Récupérer les niveaux d'analyse autorisés pour gpt_cv_generation
  let allowedAnalysisLevels = ['rapid', 'medium', 'deep']; // Par défaut tous les niveaux

  if (subscription?.plan?.featureLimits) {
    const generateCvLimit = subscription.plan.featureLimits.find(
      (limit) => limit.featureName === 'gpt_cv_generation'
    );

    if (generateCvLimit?.allowedAnalysisLevels) {
      try {
        allowedAnalysisLevels = JSON.parse(generateCvLimit.allowedAnalysisLevels);
      } catch (error) {
        console.error('[Subscriptions] Erreur lors du parsing de allowedAnalysisLevels:', error);
      }
    }
  }
