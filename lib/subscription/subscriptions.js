...
// Import statique (et non require() inline) : sûr malgré le cycle featureUsage → credits →
// subscriptions car getUserFeatureCounters n'est appelée qu'au runtime, pas au chargement du module.
import { getUserFeatureCounters } from './featureUsage';
 ...
 */
export async function assignDefaultPlan(userId, stripeCustomerId = null, options = {}) {
  try {
    ...
    if (!subscriptionEnabled) {
      ...
      const skipWelcomeCredits = options.skipWelcomeCredits === true;
      const welcomeCredits = skipWelcomeCredits ? 0 : await getNumericSettingValue('welcome_credits', 0);
      ...
      if (!existingBalance) {
        ...
        } else if (skipWelcomeCredits) {
          await prisma.creditTransaction.create({
            data: {
              userId,
              amount: 0,
              type: 'gift',
              metadata: JSON.stringify({ source: 'welcome_bonus_blocked', reason: 'duplicate_fingerprint' }),
            },
          });
          console.log(`[Subscriptions] Mode crédits-only: crédits de bienvenue BLOQUÉS pour user ${userId} (fingerprint dupliqué)`);
        }

        if (!skipWelcomeCredits) {
...
export async function getSubscriptionSummary(userId) {
  // Exécuter toutes les requêtes indépendantes en parallèle
  const [subscriptionEnabled, subscription, cvStats, creditBalance, featureCounters] = await Promise.all([
    isSubscriptionModeEnabled(),
    getUserSubscription(userId),
    getCvStats(userId),
    prisma.creditBalance.findUnique({ where: { userId } }),
    getUserFeatureCounters(userId),
  ]);
