...
/**
 * Vérifie si l'utilisateur a accepté une catégorie de cookies
 *
 * @param {string} userId - ID de l'utilisateur
 * @param {string} category - Catégorie à vérifier: "necessary", "functional", "analytics", "marketing"
 * @returns {Promise<boolean>} true si la catégorie est acceptée
 */
export async function hasConsentForCategory(userId, category) {
  if (!userId) {
    return false;
  }

  try {
    // Récupérer le dernier log de consentement de l'utilisateur
    ...
    if (!latestConsent) {
      // Pas de consentement enregistré, on refuse par défaut (opt-in RGPD)
      return false;
    }

    // Si le dernier consentement est une révocation, on refuse
    if (latestConsent.action === 'revoked') {
      return false;
    }

    // Parser les préférences
    const preferences = JSON.parse(latestConsent.preferences);
    return preferences[category] === true;
  } catch (error) {
    console.error('[ConsentLogger] Erreur lors de la vérification du consentement:', error);
    // En cas d'erreur, on refuse par défaut (RGPD)
    return false;
  }
}
