...
export async function reconstructCv(extracted) {
  return {
    ...
    experience: reconstructExperiences(normalizeSection(extracted.experience)),
    education: reconstructEducation(normalizeSection(extracted.education)),
    languages: normalizeSection(extracted.languages),
    extras: normalizeSection(extracted.extras),
    projects: reconstructProjects(normalizeSection(extracted.projects)),
  };
}
 ...
 * @returns {Object} - Summary au format standard (description uniquement)
 */
...
function reconstructExperiences(sparseExperiences) {
  return sparseExperiences.map(exp => ({
    ...
    skills_used: normalizeSection(exp.skills_used)
  }));
}
