...
export function applyPartialRollback(currentCv, change) {
  ...
  // Cas spécial: skill consolidé (multi_renamed) - plusieurs skills CV vers même skill offre
  // Quand rejeté: supprimer le skill consolidé et restaurer tous les originaux
  if (change.changeType === 'multi_renamed' && change.section === 'skills' &&
      ['hard_skills', 'tools', 'methodologies', 'soft_skills'].includes(change.field)) {
    console.log(`[applyPartialRollback] Rolling back multi_renamed skill:`, {
      field: change.field,
      afterValue: change.afterValue,
      itemsCount: change.items?.length,
    });
    ...
    // 1. Supprimer le skill consolidé
    const afterName = change.afterValue?.name?.toLowerCase().trim();
    const idx = currentSkills.findIndex(s => {
      const sName = (typeof s === 'string' ? s : s.name || '').toLowerCase().trim();
      return sName === afterName;
    });

    if (idx !== -1) {
      currentSkills.splice(idx, 1);
      console.log(`[applyPartialRollback] Removed consolidated skill: "${change.afterValue?.name}"`);
    }

    // 2. Restaurer tous les skills originaux
    if (change.items && Array.isArray(change.items)) {
      const hasProficiency = change.field === 'hard_skills' || change.field === 'tools';

      change.items.forEach(item => {
        const skillName = item.original_value;
        const alreadyExists = currentSkills.some(s => {
          const sName = (typeof s === 'string' ? s : s.name || '').toLowerCase().trim();
          return sName === skillName?.toLowerCase().trim();
        });

        if (!alreadyExists) {
          // Créer l'objet skill à restaurer
          let skillToRestore;
          if (hasProficiency) {
            skillToRestore = {
              name: skillName,
              proficiency: item.proficiency || 3, // Proficiency par défaut si manquant
            };
          } else {
            skillToRestore = skillName;
          }

          // Insérer à la position originale si disponible
          if (item.original_position !== undefined && item.original_position >= 0) {
            const insertIndex = Math.min(item.original_position, currentSkills.length);
            currentSkills.splice(insertIndex, 0, skillToRestore);
            console.log(`[applyPartialRollback] Restored original skill "${skillName}" at position ${insertIndex}`);
          } else {
            currentSkills.push(skillToRestore);
            console.log(`[applyPartialRollback] Restored original skill "${skillName}" at end`);
          }
        }
      });
    }
  ...
  // Gère aussi les skills séparés (separated_from) en restaurant le skill parent complet
  if (change.changeType === 'removed' && change.section === 'skills' &&
    ...
    console.log(`[applyPartialRollback] Restoring skills.${change.field} skill:`, {
      ...
      separatedFrom: change.separatedFrom,
    });
    ...
    if (skillToRestore) {
      ...
      // Si le skill était séparé (ex: "React" séparé de "React/Vue"),
      // restaurer le skill parent complet au lieu du skill individuel
      const actualSkillToRestore = change.separatedFrom
        ? (typeof skillToRestore === 'object'
            ? { ...skillToRestore, name: change.separatedFrom }
            : change.separatedFrom)
        : skillToRestore;

      const skillName = (typeof actualSkillToRestore === 'string'
        ? actualSkillToRestore
        : actualSkillToRestore.name || '').toLowerCase();
      ...
      if (!alreadyExists) {
        ...
        if (change.originalPosition !== undefined && change.originalPosition >= 0) {
          ...
          currentSkills.splice(insertIndex, 0, actualSkillToRestore);
          console.log(`[applyPartialRollback] Restored skill "${skillName}" to skills.${change.field} at position ${insertIndex} with proficiency:`,
            typeof actualSkillToRestore === 'object' ? actualSkillToRestore.proficiency : 'N/A',
            change.separatedFrom ? `(was separated from "${change.separatedFrom}")` : '');
        } else {
          // Fallback: ajouter à la fin si pas de position originale
          currentSkills.push(actualSkillToRestore);
