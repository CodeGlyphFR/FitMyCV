...
export async function processReviewAction(userId, filename, changeId, action) {
  ...
  // Filtrer pour ne retourner que les changements encore pending au client
  // Les changements acceptés/rejetés sont conservés en DB mais pas renvoyés
  const pendingOnly = updatedChanges.filter(c => c.status === 'pending');

  return {
    success: true,
    updatedChanges: allReviewed ? [] : pendingOnly,
    cvUpdated,
    allReviewed,
    progress: getReviewProgress(allReviewed ? [] : updatedChanges), // Progress basé sur tous les changements
  };
}
...
export async function processBatchReviewAction(userId, filename, changeIds, action) {
  ...
  return {
    ...
    allReviewed,
    processedCount,
    progress: getReviewProgress(allReviewed ? [] : pendingChanges), // Progress basé sur tous les changements
  };
}
