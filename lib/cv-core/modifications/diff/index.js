...
export function computeDetailedChanges(currentCv, previousCv, changesMade = []) {
  ...
  return changesMade.map((change) => {
    ...
    return {
      ...
      reviewedAt: null,
      // Propriétés additionnelles pour multi_renamed et review enrichi
      ...(change.items && { items: change.items }),
      ...(change.originalPosition !== undefined && { originalPosition: change.originalPosition }),
      ...(change.reviewColor && { reviewColor: change.reviewColor }),
      ...(change.popoverContent && { popoverContent: change.popoverContent }),
      ...(change.separatedFrom && { separatedFrom: change.separatedFrom }),
      ...(change.matchingProbability !== undefined && { matchingProbability: change.matchingProbability }),
    };
  });
}
