...
import { normalizeSection } from '../../normalizeSection.js';
...
export function compareExperiences(currentCv, previousCv) {
  const changes = [];
  const currentExperiences = normalizeSection(currentCv.experience);
  const previousExperiences = normalizeSection(previousCv.experience);
  const currentProjects = normalizeSection(currentCv.projects);
  const previousProjects = normalizeSection(previousCv.projects);
  ...
  currentExperiences.forEach((currentExp, currentIdx) => {
    ...
    // Skills utilisés
    const currentSkillsUsed = normalizeSection(currentExp.skills_used);
    const previousSkillsUsed = normalizeSection(previousExp.skills_used);
...
export function compareEducation(currentCv, previousCv) {
  const changes = [];
  const currentEducation = normalizeSection(currentCv.education);
  const previousEducation = normalizeSection(previousCv.education);

  const getEducationKey = (edu) => {
    if (!edu) return '';
    const institution = (edu.institution || edu.school || '').toLowerCase().trim();
    const startDate = (edu.start_date || '').toLowerCase().trim();
    const endDate = (edu.end_date || '').toLowerCase().trim();
    if (startDate || endDate) {
      return `${institution}|${startDate}|${endDate}`;
    }
    return institution;
  };
...
export function compareLanguages(currentCv, previousCv) {
  const changes = [];
  const currentLanguages = normalizeSection(currentCv.languages);
  const previousLanguages = normalizeSection(previousCv.languages);
...
export function compareExtras(currentCv, previousCv) {
  const changes = [];
  const currentExtras = normalizeSection(currentCv.extras);
  const previousExtras = normalizeSection(previousCv.extras);
