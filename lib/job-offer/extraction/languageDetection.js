 ...
 * @param {string} [params.featureName] - Feature name for telemetry (to aggregate with extraction cost)
...
export async function detectJobOfferLanguageWithOpenAI({
  ...
  const textSamples = [
    ...(extraction.responsibilities || []).slice(0, 3),
    ...(extraction.benefits || []).slice(0, 2),
  ];
  ...
  try {
    ...
    // Track usage for telemetry (uses parent featureName to aggregate cost)
    if (userId && response.usage && featureName) {
