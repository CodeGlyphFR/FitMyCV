...
import { htmlToMarkdown, extractJobOfferContent, shouldUsePuppeteerFirst, getSelectorsForUrl } from '@/lib/utils/htmlToMarkdown/index.js';
...
/**
 * Wait for SPA content to stabilize by polling body text length.
 * Returns when text length is stable (unchanged for 2 consecutive checks) and > 200 chars.
 * @param {Page} page - Puppeteer page
 * @param {number} maxWaitMs - Maximum wait time in ms (default: 10000)
 * @param {number} pollIntervalMs - Poll interval in ms (default: 500)
 */
async function waitForContentStabilization(page, maxWaitMs = 10000, pollIntervalMs = 500) {
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    if (currentLength === previousLength && currentLength > 200) {
      stableCount++;
      if (stableCount >= 2) {
        console.log(`[fetchWithPuppeteer] Content stabilized at ${currentLength} chars after ${Date.now() - startTime}ms`);
        return;
      }
    } else {
      stableCount = 0;
  ...
  console.log(`[fetchWithPuppeteer] Content stabilization timeout after ${maxWaitMs}ms (last length: ${previousLength})`);
}
...
async function fetchWithPuppeteer(url, options = {}) {
  ...
  try {
    ...
    await page.goto(url, {
      waitUntil,
      timeout: waitForContent ? 45000 : 30000,
    });
...
/**
 * Extract job offer from pre-extracted markdown content (used by browser extension)
 * Skips HTML fetch — goes straight to OpenAI structured extraction.
 *
 * @param {string} markdown - Pre-extracted markdown content
 * @param {string} title - Page title (from extension)
 * @param {string} sourceUrl - Original URL (for telemetry/cache)
 * @param {string} userId - User ID
 * @param {AbortSignal} signal - Signal to cancel the request
 * @returns {Promise<Object>} - { extraction, tokensUsed, model, title, usageDetails }
 */
export async function extractJobOfferFromMarkdown(markdown, title, sourceUrl, userId, signal = null) {
  ...
  // Check for expired/deleted pages
  const expiredCheck = detectExpiredOrDeletedPage(markdown, '', sourceUrl);
  if (expiredCheck.isExpiredPage) {
    console.log(`[extract-markdown] Expired page detected: ${expiredCheck.reason}`);
    throw new Error(JSON.stringify({
      translationKey: 'taskQueue.errors.jobOfferExpired'
    }));
  }

  // Extract with Structured Outputs (same as extractJobOfferFromUrl)
  const schema = await loadJobOfferSchema();
  const systemPrompt = await loadPrompt('lib/job-offer/prompts/system.md');
  const userPrompt = await loadPromptWithVars('lib/job-offer/prompts/user.md', {
    jobTitle: title || 'Non specifie (a extraire du contenu)',
  ...
  if (userId && response.usage) {
    await trackOpenAIUsage({
      userId,
      featureName: 'extract_job_offer_extension',
      model: extractModel,
      promptTokens: response.usage.prompt_tokens || 0,
      completionTokens: response.usage.completion_tokens || 0,
      duration,
...
/**
 * Get job offer from cache or extract from pre-extracted markdown (used by browser extension)
 * @param {string} userId - User ID
 * @param {string} markdown - Pre-extracted markdown content
 * @param {string} title - Page title
 * @param {string} sourceUrl - Original URL
 * @param {AbortSignal} signal - Signal to cancel the request
 * @returns {Promise<Object>} - { extraction, jobOfferId, title, fromCache, model, tokensUsed }
 */
export async function getOrExtractJobOfferFromMarkdown(userId, markdown, title, sourceUrl, signal = null) {
  const normalizedUrl = sourceUrl ? normalizeJobUrl(sourceUrl) : null;

  // Check if already extracted (by normalized URL) — skip if no URL (manual paste)
  if (normalizedUrl) {
    ...
    if (existing) {
      console.log(`[getOrExtractJobOfferFromMarkdown] JobOffer found in cache for URL: ${normalizedUrl}`);
      return {
        extraction: existing.content,
        jobOfferId: existing.id,
        title: existing.content?.title || title || 'Job Offer',
        fromCache: true,
        model: existing.extractionModel,
        tokensUsed: 0,
      };
    }
  }

  // Not in cache — run AI extraction
  const sourceLabel = normalizedUrl || `manual-paste-${Date.now()}`;
  console.log(`[getOrExtractJobOfferFromMarkdown] Extracting from markdown for: ${sourceLabel}`);
  const result = await extractJobOfferFromMarkdown(markdown, title, sourceUrl || sourceLabel, userId, signal);

  // Store in DB
  const isValidUrl = normalizedUrl && /^https?:\/\//i.test(normalizedUrl);
  const stored = await storeJobOffer(userId, isValidUrl ? 'url' : 'extension', normalizedUrl || sourceLabel, result.extraction, result.model, result.tokensUsed);
