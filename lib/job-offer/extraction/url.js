 ...
 * that can interfere with content fetching
...
function normalizeJobUrl(url) {
  try {
    ...
    // Indeed: remove 'from' parameter (e.g., from=shareddesktop_copy)
...
export async function getOrExtractJobOfferFromUrl(userId, url, signal = null) {
  ...
  const { extraction, tokensUsed, model, title } = await extractJobOfferFromUrl(normalizedUrl, userId, signal);
