...
export async function getOrExtractJobOfferFromMarkdown(userId, markdown, title, sourceUrl, signal = null) {
  ...
  // Check if already extracted (by normalized URL)
  ...
  console.log(`[getOrExtractJobOfferFromMarkdown] Extracting from markdown for URL: ${normalizedUrl}`);
  const result = await extractJobOfferFromMarkdown(markdown, title, sourceUrl, userId, signal);

  // Store in DB
  const isValidUrl = normalizedUrl && /^https?:\/\//i.test(normalizedUrl);
  const stored = await storeJobOffer(userId, isValidUrl ? 'url' : 'extension', normalizedUrl, result.extraction, result.model, result.tokensUsed);
