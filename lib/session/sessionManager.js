...
export async function startSession(userId) {
  ...
  // Capturer l'ancienne valeur de lastLoginAt (= connexion précédente)
  // puis l'écraser avec NOW pour le suivi d'inactivité RGPD
  ...
  try {
    ...
    previousLoginAt = user?.lastLoginAt || user?.createdAt || null;
...
async function aggregateSessionData(session) {
  const { userId, startedAt, previousLoginAt: sessionPreviousLoginAt } = session;
  ...
  // Utiliser le previousLoginAt capturé au démarrage de la session
  // (avant que signIn n'écrase lastLoginAt avec la date du login courant)
  const previousLoginAt = isNewUser ? null : sessionPreviousLoginAt;
