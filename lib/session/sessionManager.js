...
async function aggregateSessionData(session) {
  ...
  // Nouvel user = inscrit pendant cette session
  const registeredDuringSession = await prisma.telemetryEvent.findFirst({
    where: { userId, type: 'USER_REGISTERED', timestamp: { gte: startedAt, lte: endedAt } },
  });

  // Dernière visite = dernier login avant le début de cette session
  const previousLogin = await prisma.telemetryEvent.findFirst({
    where: { userId, type: 'USER_LOGIN', timestamp: { lt: startedAt } },
  ...
  return {
    user: { ...user, previousLoginAt: previousLogin?.timestamp || null, isNewUser: !!registeredDuringSession },
    session: {
      durationMs,
      features,
