...
export async function trackEvent({
  ...
  try {
    ...
    if (sessionId) {
      await updateSessionActivity(sessionId, type);
    }
 ...
 * Reuses sessions that are still active (last activity < 10 minutes)
 */
export async function findOrCreateSession({
  ...
  try {
    if (!userId) {
      // No user ID, can't find/create session
      return null;
    }

    // Look for an active session in the last 10 minutes
    ...
    if (activeSession) {
      ...
      console.log('[Telemetry] Reusing existing session:', activeSession.id);
    ...
    console.log('[Telemetry] Created new session:', session.id);
    return session;
  } catch (err) {
    console.error('[Telemetry] Failed to find/create session:', err);
    return null;
  }
}
 ...
 * @deprecated Use findOrCreateSession instead to avoid duplicate sessions
 */
 ...
 */
export async function updateSessionActivity(sessionId, eventType = null) {
  try {
    ...
    // Increment pagesViewed for PAGE_VIEW events
    if (eventType === 'PAGE_VIEW') {
      updateData.pagesViewed = {
        increment: 1,
      };
    }
 ...
 * - Have been inactive for more than 10 minutes
 * - OR have been open for more than 24 hours (max lifetime)
 * @returns {Promise<{closed: number, total: number}>} Number of sessions closed
 */
export async function cleanupInactiveSessions() {
  try {
    ...
    // 1. Inactive for > 10 minutes
    // 2. OR open for > 24 hours (max lifetime)
    ...
    console.log(`[Telemetry] Closed ${result.count} sessions (inactive or > 24h)`);
    return { closed: result.count, total: sessionsToClose.length };
  } catch (err) {
    console.error('[Telemetry] Failed to cleanup inactive sessions:', err);
