...
/**
 * Check if Priority mode is enabled for OpenAI pricing
 * @returns {Promise<boolean>} - True if priority mode is enabled
 */
async function isPriorityModeEnabled() {
  try {
    ...
    console.warn('[OpenAI Telemetry] Failed to check priority mode, defaulting to false:', error.message);
    return false;
  }
}
...
export async function calculateCost({
  ...
  try {
    ...
    const usePriority = await isPriorityModeEnabled();

    // Select pricing based on mode (fallback to standard if priority prices not set)
    const inputPrice = usePriority && pricing.inputPricePerMTokenPriority != null
      ? pricing.inputPricePerMTokenPriority
      : pricing.inputPricePerMToken;
    const cachePrice = usePriority && pricing.cachePricePerMTokenPriority != null
      ? pricing.cachePricePerMTokenPriority
      : pricing.cachePricePerMToken;
    const outputPrice = usePriority && pricing.outputPricePerMTokenPriority != null
      ? pricing.outputPricePerMTokenPriority
    ...
    const inputCost = (nonCachedPromptTokens / 1_000_000) * inputPrice;
...
/**
 * Check if any alert thresholds have been reached or exceeded
 *
  ...
  try {
    ...
    for (const alert of alerts) {
      ...
      switch (alert.type) {
        ...
        case 'user_monthly': {
          // Check user's total cost for their monthly period
          // HYBRID: Uses Stripe billing period if available, otherwise calendar month
          const monthlyPeriod = await getUserMonthlyPeriod(userId);
          if (monthlyPeriod) {
            const userMonthlyCost = await prisma.openAIUsage.aggregate({
              where: {
                userId,
                date: {
                  gte: monthlyPeriod.start,
                  lte: monthlyPeriod.end,
                },
              },
              _sum: {
                estimatedCost: true,
