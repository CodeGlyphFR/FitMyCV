...
import { getOpenAIClient, checkOpenAICredits, addTemperatureIfSupported, addTopPIfSupported, addSeedIfSet } from '@/lib/openai-core/client';
import { getAiModelSetting } from '@/lib/settings/aiModels';
import { loadPromptWithVars } from '@/lib/openai-core/promptLoader';
import { trackOpenAIUsage } from '@/lib/telemetry/openai';
import { getPdfPageCount, convertPdfToImages, getPdfImageConfig } from '@/lib/openai-core/pdfToImages';
import { reconstructCv } from '@/lib/cv-core/reconstruction';
 ...
 * @param {number} temperature - Température GPT (0-2)
 * @param {number} topP - Top P / nucleus sampling (0-1)
 * @param {number} seed - Seed pour reproductibilité (0 = désactivé)
 * @returns {Promise<Object>} - Contenu et métadonnées de la réponse
 */
async function callChatGPTWithVision(client, model, imageBase64Array, visionDetail = 'high', signal = null, temperature = 0.1, topP = 1, seed = 0) {
  try {
    ...
    // Charger les prompts avec variables injectées (la langue est détectée par le modèle dans le reasoning)
    const systemPrompt = await loadPromptWithVars('lib/features/import-pdf/prompts/system.md', {
      cvSchema
    });
    const userTextPrompt = await loadPromptWithVars('lib/features/import-pdf/prompts/user.md', {});
    ...
    console.log(`[importPdf] Envoi de ${imageBase64Array.length} image(s) à Vision (detail: ${visionDetail}, temp: ${temperature}, top_p: ${topP}, seed: ${seed || 'off'})...`);
    ...
    const requestOptions = {
      ...
      max_tokens: 16384 // Marge large pour les CV denses multi-pages
    };

    // Ajouter température, top_p et seed si supportés/configurés
    addTemperatureIfSupported(requestOptions, temperature);
    addTopPIfSupported(requestOptions, topP);
    addSeedIfSet(requestOptions, seed);
...
export async function importPdfCv({ pdfFilePath, userId = null, isFirstImport = false, signal = null }) {
  ...
  // 6. Appeler GPT-4o Vision avec schéma d'extraction (la langue est détectée dans le reasoning)
  console.log(`[importPdf] Analyse du CV par Vision API (temp: ${pdfConfig.temperature}, top_p: ${pdfConfig.topP}, seed: ${pdfConfig.seed})...`);
  const result = await callChatGPTWithVision(client, model, imageBase64Array, pdfConfig.detail, signal, pdfConfig.temperature, pdfConfig.topP, pdfConfig.seed);
  ...
  // 7. Parser le contenu extrait et sanitizer les null bytes (PostgreSQL ne les supporte pas)
  const extracted = removeNullBytes(JSON.parse(result.content.trim()));

  // 8. Récupérer la langue détectée par le modèle dans le reasoning
  const language = extracted.reasoning?.detected_language || 'fr';
  console.log(`[importPdf] Langue détectée par le modèle : ${language}`);
