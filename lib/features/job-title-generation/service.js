import { getOpenAIClient, checkOpenAICredits } from '@/lib/openai-core/client';
...
async function getCvSchema() {
  ...
  // Fallback: schéma par défaut (contenu pur, métadonnées en DB)
...
async function validateJobTitle(jobTitle, model, userId = null) {
  ...
  const systemPrompt = await loadPrompt('lib/features/job-title-generation/prompts/validate-job-title/system.md');
  const userPrompt = await loadPromptWithVars('lib/features/job-title-generation/prompts/validate-job-title/user.md', {
    jobTitle: jobTitle
  });
  ...
  try {
    ...
    // Tracking OpenAI usage (validation call)
    ...
    throw new Error(JSON.stringify({ translationKey: 'errors.api.openai.jobTitleValidationFailed' }));
  }
}
...
export async function generateCvFromJobTitle({
  ...
  if (!jobTitle || typeof jobTitle !== 'string' || !jobTitle.trim()) {
    throw new Error(JSON.stringify({ translationKey: 'errors.api.openai.jobTitleMissing' }));
  }
  ...
  // Charger les prompts depuis les fichiers .md
  const systemPrompt = await loadPrompt('lib/features/job-title-generation/prompts/generate-from-job-title/system.md');
  const userPrompt = await loadPromptWithVars('lib/features/job-title-generation/prompts/generate-from-job-title/user.md', {
  ...
  try {
    ...
    if (!parsedCV.header?.full_name || !parsedCV.header?.current_title) {
      throw new Error(JSON.stringify({ translationKey: 'errors.api.openai.incompleteCv' }));
    }
