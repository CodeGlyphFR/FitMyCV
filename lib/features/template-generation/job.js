...
// Import dynamique pour eviter de charger les modules lourds au demarrage
async function getCreateTemplateCv() {
  const module = await import("@/lib/features/template-generation/service");
...
export async function runCreateTemplateCvJob({
  ...
  try {
    ...
    console.warn(`Impossible de verifier la tache ${taskId} avant demarrage`, error);
  }

  // Creer un AbortController pour pouvoir annuler la tache
  ...
  // Preparation des fichiers joints
  ...
  try {
    // Verifier si annule avant l'operation longue
    if (abortController.signal.aborted) {
    ...
    // Callback pour mettre a jour le titre des que l'offre est extraite
    const onTitleExtracted = async (title) => {
      if (title) {
        console.log(`[createTemplateCvJob] Titre extrait: ${title}`);
        await updateBackgroundTask(taskId, userId, {
          title: title,
        });
      }
    ...
    generatedContents = await createTemplateCv({
      ...
      userId, // Pour la telemetrie OpenAI
      onTitleExtracted,
      markdownContent: payload.markdownContent || null,
      markdownTitle: payload.markdownTitle || null,
    });

    // Verifier si annule apres l'operation
    if (abortController.signal.aborted) {
      throw new Error('Task cancelled');
    }
  } catch (error) {
    ...
    if (error.message === 'Task cancelled' || abortController.signal.aborted) {
      console.log(`[createTemplateCvJob] Tache ${taskId} annulee`);
    ...
    console.error(`[createTemplateCvJob] Erreur lors de la creation de CV modele pour la tache ${taskId}:`, error);
    ...
    const errorMessage = isQuotaExceeded
      ? "Quota OpenAI depasse. Verifiez votre facturation."
      : (error.message || 'Echec lors de la creation du CV modele');

    console.error(`[createTemplateCvJob] Message d'erreur retourne: ${errorMessage}`);
    ...
    try {
      ...
      console.error('[createTemplateCvJob] Erreur tracking telemetrie:', trackError);
    }

    return;
  }

  // Sauvegarde des CV generes
  const createdFiles = [];
  ...
  for (let i = 0; i < generatedContents.length; i++) {
    ...
    try {
      // Enrichissement avec metadonnees
      ...
      try {
        ...
        console.error(`Impossible d'enrichir ${filename} avec les metadonnees`, metaError);
      }

      // Valider le contenu avant de sauvegarder
      if (!validateCvContent(enriched)) {
        console.warn(`[createTemplateCvJob] CV vide detecte : ${filename}`);
        hasEmptyCv = true;
        continue;
      }

      // Langue du CV = langue de l'offre d'emploi (extraite lors de extractJobOffer)
      const detectedLanguage = result.jobOfferLanguage || null;
      if (detectedLanguage) {
        console.log(`[createTemplateCvJob] Langue de l'offre utilisee: ${detectedLanguage}`);
      } else {
        console.log(`[createTemplateCvJob] Langue de l'offre non disponible`);
      }

      await writeUserCvFile(userId, filename, enriched);
      createdFiles.push(filename);

      // Determiner le type de source (exclure les pseudo-URLs manual://)
      const rawSourceUrl = Array.isArray(payload.links) && payload.links.length > 0 ? payload.links[i] || payload.links[0] : null;
      const sourceUrl = rawSourceUrl && !rawSourceUrl.startsWith('manual://') ? rawSourceUrl : null;
      const sourceUpload = Array.isArray(payload.uploads) && payload.uploads.length > 0 ? payload.uploads[0].name : null;

      // Creer/Mettre a jour l'entree CvFile avec metadonnees (langue incluse)
      ...
      // Enregistrer la source (pour compatibilite)
      if (sourceUrl) {
        try {
      ...
      console.log(`[createTemplateCvJob] CV modele cree avec succes : ${filename}${result.jobOfferId ? ' (avec extraction offre)' : ''}`);

      // Tracking telemetrie - Succes pour ce CV
      const duration = Date.now() - startTime;
  ...
  // Si tous les CV generes sont vides, marquer la tache comme echouee
  if (hasEmptyCv && createdFiles.length === 0) {
    await updateBackgroundTask(taskId, userId, {
      status: 'failed',
      result: null,
      error: 'Aucun contenu type CV detecte.',
    });
    return;
  }
