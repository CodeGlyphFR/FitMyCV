...
export async function createTemplateCv({
  ...
  if (links?.length > 0) {
    for (let i = 0; i < links.length; i++) {
      ...
      try {
        // Extract job offer: use markdown for manual offers, URL for real links
        let jobOffer;
        if (isManualOffer && markdownContent) {
          console.log(`[createTemplateCv] Extraction depuis contenu manuel colle`);
          jobOffer = await getOrExtractJobOfferFromMarkdown(userId, markdownContent, markdownTitle || '', null, signal);
        } else if (isManualOffer) {
          throw new Error('Manual offer without content');
        } else {
          jobOffer = await getOrExtractJobOfferFromUrl(userId, url, signal);
        }

        if (jobOffer.fromCache) {
          console.log(`[createTemplateCv] Skipped OpenAI extraction (cached): ${url}`);
        }

        // Notifier le titre extrait immediatement
        if (onTitleExtracted && jobOffer.title) {
          try {
            await onTitleExtracted(jobOffer.title);
          } catch (callbackError) {
            console.error('[createTemplateCv] Erreur callback onTitleExtracted:', callbackError);
          }
        }

        const displaySource = isManualOffer ? (jobOffer.title || 'Offre collee manuellement') : url;
        const currentOfferContent = formatJobOfferForTemplate(jobOffer.extraction, displaySource, jobOffer.title);
        console.log(`\n[createTemplateCv] Creation de CV modele pour : ${displaySource}`);
        ...
        generatedContents.push({
          ...
          jobOfferLanguage: jobOffer.extraction?.language || null,
          jobOfferTitle: jobOffer.title || null,
