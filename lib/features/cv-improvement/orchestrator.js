...
import { normalizeToNumber } from '@/lib/constants/skillLevels';
...
export async function runImprovementPipeline({
        ...
        if (existingIndex === -1) {
          ...
          allChangesMade.push({
            ...
            afterValue: category === 'hard_skills' || category === 'tools'
              ...
              : skillName,
            change: `Ajout de ${skillName} (niveau ${level}/5)`,
  ...
  if (!preprocessResult.success || !preprocessResult.data) {
    ...
    for (const result of stage3Results) {
      ...
      if (result.type === 'experience') {
      ...
      } else if (result.type === 'languages' && (result.data.hasChanges || result.data.modifications?.levelChanges?.length || result.data.modifications?.certificationChanges?.length || result.data.modifications?.newLanguages?.length || result.data.modifications?.reorder?.length)) {
        // Appliquer les modifications de langues (hasChanges peut être undefined même si des modifications existent)
        const { applyLanguageModifications } = await getImproveLanguages();
        const originalLanguages = JSON.stringify(workingCv.languages);

        // Capturer les noms de langue AVANT le réordonnancement pour le tracking
        const languageNamesBeforeReorder = normalizeSection(workingCv.languages).map(l => l?.name || '');
        ...
        if (result.data.modifications.levelChanges?.length) {
          for (const levelChange of result.data.modifications.levelChanges) {
            // Utiliser le nom capturé AVANT réordonnancement
            const langName = languageNamesBeforeReorder[levelChange.languageIndex] || `Langue ${levelChange.languageIndex}`;
            console.log(`[orchestrator] Language level change tracked: index=${levelChange.languageIndex}, name="${langName}", ${levelChange.oldLevel} → ${levelChange.newLevel}`);
            allChangesMade.push({
              section: 'languages',
              field: 'level',
        ...
        if (result.data.modifications.certificationChanges?.length) {
          for (const certChange of result.data.modifications.certificationChanges) {
            // Utiliser le nom capturé AVANT réordonnancement
            const langName = languageNamesBeforeReorder[certChange.languageIndex] || `Langue ${certChange.languageIndex}`;
            allChangesMade.push({
              section: 'languages',
              field: 'certification',
