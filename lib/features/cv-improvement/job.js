...
import { computeCvDiff } from "@/lib/cv-core/modifications/diff";
...
export async function runImproveCvJob({
  ...
  try {
    ...
    // Pipeline (4 stages)
    console.log(`[improveCvJob] Using Pipeline (4 stages)`);
    ...
    // Debug: Log les changements de langues
    const programmaticLangChanges = programmaticChanges.filter(c => c.section === 'languages');
    const orchestratorLangChanges = allChangesMade.filter(c => c.section === 'languages');
    console.log(`[improveCvJob] Changements langues - programmatic: ${programmaticLangChanges.length}, orchestrator: ${orchestratorLangChanges.length}`);
    if (programmaticLangChanges.length > 0) {
      programmaticLangChanges.forEach(c => console.log(`[improveCvJob]   programmatic: itemName="${c.itemName}", changeType=${c.changeType}`));
    }
    if (orchestratorLangChanges.length > 0) {
      orchestratorLangChanges.forEach(c => console.log(`[improveCvJob]   orchestrator: itemName="${c.itemName}", changeType=${c.changeType}`));
    }

    let changesMade;
    let hasRealChanges = false;  // Flag pour savoir si on a de vrais changements
    if (programmaticChanges.length > 0) {
      changesMade = programmaticChanges;
      hasRealChanges = true;
      console.log(`[improveCvJob] Using programmaticChanges (computeCvDiff)`);
    } else if (allChangesMade.length > 0) {
      changesMade = allChangesMade;
      hasRealChanges = true;
      console.log(`[improveCvJob] Using allChangesMade (orchestrator)`);
    } else {
      // Pas de vrais changements détectés → CV déjà optimisé
      hasRealChanges = false;
      console.log(`[improveCvJob] Aucun changement détecté — CV déjà optimisé`);

      // Remettre le statut du CV à idle
      await updateCvFile(userId, cvFile, { optimiseStatus: 'idle', optimiseUpdatedAt: new Date() });
      ...
      // Marquer la tâche comme completed avec message "déjà optimisé"
      const noChangeMessage = JSON.stringify({ key: 'taskQueue.messages.improvementNoChanges' });
      await updateBackgroundTask(taskId, userId, {
        status: 'completed',
        ...
        error: null,
        successMessage: noChangeMessage,
      });

      // Émettre SSE pour notifier l'UI
    ...
    // N'initialiser le review state que s'il y a de vrais changements (pas le fallback)
    if (replaceExisting && createdVersion !== null && hasRealChanges && changesMade.length > 0) {
      try {
      ...
      console.log(`[improveCvJob] Pas de vrais changements - review state non initialisé`);
    }
    ...
    const successMessage = replaceExisting
      ? JSON.stringify({ key: 'taskQueue.messages.improvementReplacedCompleted', params: { count: changesCount } })
      : JSON.stringify({ key: 'taskQueue.messages.improvementCompleted', params: { count: changesCount } });
