...
export async function improveExtras({
  ...
  // Charger les prompts
  const systemPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/extras-system.md', {
    cvLanguage: cvLanguageName,
  });
  const userPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/extras-user.md', {
    ...
    if (userId && response.usage) {
      try {
        await trackOpenAIUsage({
          userId,
          featureName: 'cv_improvement_extras',
          model,
...
export function applyExtrasModifications(extras, result) {
  extras = normalizeSection(extras);
