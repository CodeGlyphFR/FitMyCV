...
export async function improveProject({
  ...
  // Charger les prompts
  const systemPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/project-system.md', {
    cvLanguage: cvLanguageName,
  });
  const userPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/project-user.md', {
    ...
    if (userId && response.usage) {
      try {
        await trackOpenAIUsage({
          userId,
          featureName: 'cv_improvement_v2_project',
          model,
