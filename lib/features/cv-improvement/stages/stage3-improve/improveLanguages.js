...
export async function improveLanguages({
  ...
  // Normaliser les langues (vieilles données possiblement au format string)
  languages = normalizeSection(languages);
  ...
  // Charger les prompts
  const systemPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/languages-system.md', {
    cvLanguage: cvLanguageName,
  });
  const userPrompt = await loadPromptWithVars('lib/features/cv-improvement/prompts/languages-user.md', {
  ...
  try {
    ...
    console.log('[improveLanguages] Réponse IA brute:', JSON.stringify(result, null, 2));
    console.log('[improveLanguages] hasChanges (from AI):', result.hasChanges);
    ...
    if (userId && response.usage) {
      try {
        await trackOpenAIUsage({
          userId,
          featureName: 'cv_improvement_languages',
          model,
    ...
    // Calculer hasChanges à partir des modifications si l'IA ne le retourne pas
    const mods = result.modifications || {};
    const computedHasChanges = !!(
      mods.levelChanges?.length ||
      mods.certificationChanges?.length ||
      mods.newLanguages?.length ||
      mods.reorder?.length
    );
    const finalHasChanges = result.hasChanges === true || computedHasChanges;
    console.log('[improveLanguages] hasChanges (computed):', finalHasChanges);
