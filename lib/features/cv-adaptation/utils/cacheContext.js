...
export function generateCacheA(jobOffer) {
  ...
  return `# OFFRE D'EMPLOI CIBLE (contexte uniquement)

## ⚠️ REGLES CRITIQUES - TITRE DES EXPERIENCES
- Le titre "${keyInfo.title}" est le poste VISE par le candidat
- INTERDIT de copier ce titre dans les experiences
- Chaque experience garde son PROPRE titre (celui que le candidat a eu)
- Le titre doit refleter ce que le candidat A FAIT, pas ce qu'il VISE

## Autres regles
- NE PAS ajouter de mots-cles de cette offre si l'experience ne les contient pas

**Poste vise (CONTEXTE SEULEMENT):** ${keyInfo.title}
