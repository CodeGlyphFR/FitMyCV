  ...
  const processCategory = (items) => {
    ...
    return items.map(item => {
      ...
      if (item.action === 'deleted') {
        ...
        // Skill conservé → review si traduit (langues différentes ET nom réellement changé)
        if (languagesDifferent) {
          const nameChanged = item.original_value?.toLowerCase().trim() !== item.skill_final?.toLowerCase().trim();
          if (nameChanged) {
            reviewRequired = true;
            reviewColor = 'amber';
          }
