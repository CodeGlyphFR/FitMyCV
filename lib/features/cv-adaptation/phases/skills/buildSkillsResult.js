...
function consolidateDuplicateSkillFinals(categoryResult, hasProficiency) {
  ...
  // Regrouper les skills "renamed" ou "kept" par skill_final (case-insensitive)
  // Cela permet de consolider des cas comme:
  // - "Agile" (kept, score 100) + "Lean Startup" → "Agile" (renamed, score 70)
  // = 1 seul "Agile" consolidé au lieu de 2
  const bySkillFinal = new Map();

  categoryResult.forEach((skill, idx) => {
    // Les skills "renamed" ET "kept" sont candidats à la consolidation
    if ((skill.action === 'renamed' || skill.action === 'kept') && skill.skill_final) {
  ...
  bySkillFinal.forEach((skills, key) => {
    if (skills.length >= 2) {
      // Vérifier qu'il y a au moins un "renamed"
      // Si tous sont "kept", c'est le même skill original donc pas de consolidation
      const hasRenamed = skills.some(s => s.action === 'renamed');
      if (!hasRenamed) {
        return; // Tous kept = pas de consolidation nécessaire
      }
