...
import { initializeReviewState } from '@/lib/cv-core/review/state';
import { generateChangeId, computeCvDiff, computeArrayItemDiff } from '@/lib/cv-core/modifications/diff';
...
function extractChangesFromBatchResults(batchResults, sourceCv) {
  ...
  if (skillsRaw) {
    ...
    for (const category of categories) {
      ...
      // Collecter les noms originaux des skills consolidés pour éviter les doublons "removed"
      const consolidatedOriginalNames = new Set();
      for (const item of skillItems) {
        if (item.consolidated_from && Array.isArray(item.consolidated_from)) {
          for (const c of item.consolidated_from) {
            if (c.original_value) {
              consolidatedOriginalNames.add(c.original_value.toLowerCase().trim());
            }
          }
        }
      }
      ...
      for (const item of skillItems) {
        ...
        if (item.action === 'renamed') {
          ...
          // Skip si ce skill fait partie d'une consolidation (déjà géré par multi_renamed)
          if (consolidatedOriginalNames.has(item.original_value?.toLowerCase().trim())) {
            continue;
          }
          // Skill supprimé
          changeType = 'removed';
  ...
  if (batchResults.language_modifications && Array.isArray(batchResults.language_modifications)) {
    for (const langMod of batchResults.language_modifications) {
      ...
      // Vérifier les valeurs réelles plutôt que se fier à l'action de l'IA
      // (l'IA peut marquer "kept" des langues dont le nom/niveau a changé)
 ...
 */
function generateCvFileName() {
  return `${DateTime.now().toFormat('yyyyMMddHHmmssSSS')}.json`;
}
...
export async function executeRecomposition({
  ...
  // Note: jobOffer est le contenu JSON de l'offre (extraction AI), pas le record Prisma complet
  ...
  try {
    ...
    const generatedFileName = generateCvFileName();
    ...
    // Charger le record JobOffer pour les metadonnees (sourceValue, sourceType, extractedAt)
    // Note: jobOffer param est le contenu JSON (extraction), pas le record DB complet
    let jobOfferRecord = null;
    if (jobOfferId) {
      jobOfferRecord = await prisma.jobOffer.findUnique({
        where: { id: jobOfferId },
        select: { sourceType: true, sourceValue: true, extractedAt: true },
      });
    }
    ...
    const cvFile = await prisma.cvFile.create({
      data: {
        ...
        jobOfferSnapshot: jobOfferContent ? {
          ...
          sourceValue: jobOfferRecord?.sourceValue || offerId,
          extractedAt: jobOfferRecord?.extractedAt ? new Date(jobOfferRecord.extractedAt).toISOString() : new Date().toISOString(),
    ...
    try {
      ...
      // Créer un Set des noms "avant" des changements modified dont beforeValue est un objet
      // (langues, extras, education) - ces éléments sont détectés comme "removed" par le diff
      // programmatique car le nom a changé (traduction), mais l'IA les a déjà correctement
      // identifiés comme "modified"
      const modifiedObjectBeforeNames = new Set(
        aiChanges
          .filter(c => c.changeType === 'modified' && c.beforeValue && typeof c.beforeValue === 'object')
          .map(c => {
            const beforeName = c.beforeValue.name || c.beforeValue.title || '';
            if (!beforeName) return null;
            return `${c.section}|${c.field}|${beforeName}|${c.expIndex ?? ''}`.toLowerCase();
          })
          .filter(Boolean)
      );
      ...
      // Collecter les noms originaux des skills consolidés (multi_renamed) pour éviter les doublons
      const consolidatedOriginalKeys = new Set();
      aiChanges.forEach(c => {
        if (c.changeType === 'multi_renamed' && c.items) {
          c.items.forEach(item => {
            const key = `${c.section}|${c.field}|${item.original_value}|`.toLowerCase();
            consolidatedOriginalKeys.add(key);
          });
        }
      });
      ...
      // 2. Ceux dont l'itemName correspond au beforeValue d'un changement modified (traductions skills)
      // 3. Ceux dont l'itemName correspond au nom original d'un skill supprimé par l'IA
      // 4. Ceux dont l'itemName correspond à un skill original d'une consolidation multi_renamed
      // 5. Ceux dont l'itemName correspond au beforeValue.name d'un changement modified (langues/extras/education)
      const uniqueRemovedItems = removedItems.filter(c => {
        const key = `${c.section}|${c.field}|${c.itemName || ''}|${c.expIndex ?? ''}`.toLowerCase();
        return !aiChangeKeys.has(key) && !modifiedBeforeValues.has(key) && !removedOriginalNames.has(key) && !consolidatedOriginalKeys.has(key) && !modifiedObjectBeforeNames.has(key);
      });
      ...
      // mais remonter un avertissement dans le statut de la tâche
      ...
      try {
        await prisma.cvGenerationSubtask.update({
          ...
          data: {
            output: {
              warning: `Review state initialization failed: ${reviewError.message}`,
            },
          },
        });
      } catch { /* best effort */ }
