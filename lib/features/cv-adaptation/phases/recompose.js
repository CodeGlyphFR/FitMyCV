  ...
  if (batchResults.skills?.modifications && Array.isArray(batchResults.skills.modifications)) {
    ...
    for (const mod of batchResults.skills.modifications) {
      ...
      if (mod.action === 'modified') {
        ...
        // Utiliser la traduction si disponible pour le rollback
        beforeValue = mod.translated || mod.before;
        beforeDisplay = mod.translated || mod.before;
        itemName = mod.translated || mod.before;
        changeDescription = `"${mod.translated || mod.before}" supprime`;
