...
function extractChangesFromBatchResults(batchResults, sourceCv) {
    ...
    batchResults.experiences.forEach((adaptedExp, expIndex) => {
      ...
      if (adaptedExp.skills_modifications && Array.isArray(adaptedExp.skills_modifications)) {
        for (const skillMod of adaptedExp.skills_modifications) {
          ...
          // Pour les skills supprimés, utiliser la traduction si disponible pour le rollback
          // Cela permet de restaurer le skill dans la langue cible si l'utilisateur refuse la suppression
          if (skillMod.action === 'removed' && skillMod.translated) {
            beforeValue = skillMod.translated;
          }
          ...
          changes.push({
            ...
            beforeValue,
            afterValue,
            beforeDisplay: skillMod.before || '',
            afterDisplay: skillMod.after || '',
            change: changeType === 'modified'
              ? `Skill reformule: "${skillMod.before}" → "${skillMod.after}"`
              : changeType === 'removed'
              ? `Skill supprime: "${skillMod.before}"`
              : `Skill ajoute: "${skillMod.after}"`,
  ...
  if (batchResults.skills?.modifications && Array.isArray(batchResults.skills.modifications)) {
    for (const mod of batchResults.skills.modifications) {
      ...
      // Récupérer l'objet complet depuis sourceCv
      const sourceSkillObj = sourceCategory.find(s => {
        const sName = typeof s === 'string' ? s : s.name || '';
        return sName.toLowerCase() === mod.skill.toLowerCase();
      });

      // Récupérer le skill adapté (avec le nouveau proficiency) depuis batchResults
      const adaptedSkillObj = adaptedCategory.find(s => {
        const sName = typeof s === 'string' ? s : s.name || '';
        return sName.toLowerCase() === mod.skill.toLowerCase();
      });

      // Déterminer le changeType selon l'action de l'IA
      let changeType;
      let itemValue = null;
      ...
      if (mod.action === 'removed') {
        ...
        beforeValue = sourceSkillObj || mod.skill;
        beforeDisplay = mod.skill;
        changeDescription = `"${mod.skill}" supprime`;
        ...
        // Récupérer le nouveau niveau depuis le skill adapté
        ...
        changeDescription = `"${mod.skill}" niveau ajuste (${oldProficiency} → ${newProficiency})`;
      } else {
        // added, renamed, split, moved, merged, inferred
        changeType = 'added';
        afterValue = mod.skill;
        afterDisplay = mod.skill;
        changeDescription = `"${mod.skill}" ajoute`;
      }
...
export async function executeRecomposition({
  ...
  try {
    ...
    try {
      ...
      // Detecter les suppressions via diff programmatique
      const programmaticChanges = computeCvDiff(adaptedCv, sourceCv);

      // Filtrer pour ne garder que les suppressions (removed, experience_removed)
      ...
      const modifiedBeforeValues = new Set(
        aiChanges
          .filter(c => c.changeType === 'modified' && c.field === 'skills_used' && c.beforeValue)
          .map(c => {
      ...
      const uniqueRemovedItems = removedItems.filter(c => {
        ...
        // Exclure si déjà dans aiChanges ou si c'est une valeur "avant" d'une modification
        return !aiChangeKeys.has(key) && !modifiedBeforeValues.has(key);
      });
      console.log(`[recompose] After dedup: ${uniqueRemovedItems.length} unique removed item(s)`);

      // Fusionner : changements IA + suppressions detectees (sans doublons)
      const changesMade = [...aiChanges, ...uniqueRemovedItems];
      console.log(`[recompose] Total changes for review: ${changesMade.length} (${aiChanges.length} AI + ${uniqueRemovedItems.length} removed)`);
