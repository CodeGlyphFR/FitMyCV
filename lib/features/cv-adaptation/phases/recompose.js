...
export async function executeRecomposition({
  ...
  try {
    ...
    try {
      ...
      // Fusionner : changements IA + suppressions detectees
      const changesMade = [...aiChanges, ...removedItems];
      console.log(`[recompose] Total changes for review: ${changesMade.length} (${aiChanges.length} AI + ${removedItems.length} removed)`);
