  ...
  try {
    ...
    try {
      ...
      const uniqueRemovedItems = removedItems.filter(c => {
        ...
        // si c'est le nom original d'un skill supprimé par l'IA,
        // ou si c'est un skill original d'une consolidation multi_renamed
        return !aiChangeKeys.has(key) && !modifiedBeforeValues.has(key) && !removedOriginalNames.has(key) && !consolidatedOriginalKeys.has(key);
      });
