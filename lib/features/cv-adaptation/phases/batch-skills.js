...
/**
 * Formate une liste de skills pour l'affichage dans le prompt
 * @param {Array} skills - Liste des skills
 * @returns {string} - Liste formatee avec puces ou "Non specifie"
 */
...
function extractJobOfferInfo(jobOffer) {
  ...
  // Extraire toutes les compétences requises (combinées pour le prompt)
  const allRequired = [
  ...
  return {
    ...
    // Variables pour le filtrage strict
    requiredSkillsList: formatSkillsList(allRequired),
    niceToHaveSkillsList: formatSkillsList(allNiceToHave),
    softSkillsList: formatSkillsList(softSkills),
  };
}
 ...
 * @param {string} params.sourceLanguage - Langue du CV source
...
export async function executeBatchSkills({
  ...
  try {
    ...
    // Calculer si traduction necessaire
    const needsTranslation = sourceLanguage !== targetLanguage ? 'OUI' : 'NON';

    // Extraire les listes de skills de l'offre pour le filtrage strict
    ...
    const userPrompt = replaceVariables(userPromptTemplate, {
      ...
      targetLanguage,
      needsTranslation,
      // Nouvelles variables pour le filtrage strict
      requiredSkillsList: jobOfferInfo.requiredSkillsList,
      niceToHaveSkillsList: jobOfferInfo.niceToHaveSkillsList,
      softSkillsList: jobOfferInfo.softSkillsList,
    });
