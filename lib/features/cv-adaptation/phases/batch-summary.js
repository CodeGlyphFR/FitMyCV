 ...
 * - Si < 1 an : affiche en mois (ex: "6 mois")
...
function aggregateExperiencesByDomain(experiences) {
  ...
  // Construire le resume textuel avec format adapte
  const domainsSummary = sortedDomains
    .map(([domain, data]) => {
      const years = Math.round(data.totalYears * 10) / 10;
      // Afficher en mois si < 1 an
      if (years < 1) {
        const months = Math.round(years * 12);
        return `- ${domain}: ${months} mois`;
      }
      // Arrondi half-down pour l'affichage >= 1 an
      const roundedYears = roundHalfDown(years);
      return `- ${domain}: ${roundedYears} an${roundedYears > 1 ? 's' : ''}`;
    })
    .join('\n');

  // Details pour le JSON (garde la precision decimale)
...
export async function executeBatchSummary({
  ...
  try {
    ...
    // Créer les modifications dans le format attendu par recompose.js
    const modifications = [
      {
        field: 'description',
        action: 'generated',
        reason: result.reason || 'Adaptation au poste ciblé',
      }
    ];
