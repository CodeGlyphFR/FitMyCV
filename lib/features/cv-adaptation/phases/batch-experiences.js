 ...
 *
 * Champ SUGGERE par l'IA (compare avec l'original):
 * - title: titre ATS-friendly suggere, applique seulement si different
 *
...
import { writeDebugLog, LogType, generateTimestamp } from '../utils/debugLog.js';
...
/**
 * Arrondi "half-down" : X.5 → X, X.6 → X+1
 * Exemple: 4.4 → 4, 4.5 → 4, 4.6 → 5
 * @param {number} value - Valeur a arrondir
 ...
 * - Si >= 1 an : arrondi half-down a l'entier (4.5 → 4, 4.6 → 5)
 * - Si < 1 an : garde la valeur decimale (sera affiche en mois)
 * @param {string} startDate - Date de debut (format YYYY-MM ou YYYY)
 * @param {string} endDate - Date de fin ou "present"/"Present"
 * @returns {number} - Duree en annees (entier si >= 1 an, decimal si < 1 an)
 */
...
function sanitizeAdaptedExperience(adaptedExperience, originalExperience) {
  ...
  // Comparer le titre suggere par l'IA avec l'original
...
async function adaptSingleExperience({
  ...
  try {
    ...
    // NOTE: Le titre est EXCLU volontairement pour ne pas biaiser la determination du domaine et du titre ATS
    ...
    // Ajuster max_completion_tokens pour les modèles de raisonnement (réduit de 16000 à 8000)
    requestOptions = adjustTokensForReasoningModel(requestOptions, 1500, 8000);
    ...
    await writeDebugLog({
      type: LogType.EXPERIENCES,
      filename: `experience_${experienceIndex}.md`,
      generationTimestamp,
      systemPrompt,
      userPrompt,
      response: rawExperience,
      duration,
      title: `Debug Experience Batch: Experience ${experienceIndex}`,
    });
 ...
 * @param {string} params.userId - ID utilisateur
 * @param {AbortSignal} params.signal - Signal d'annulation
 * @param {string} params.debugTimestamp - Timestamp partagé pour les logs de debug
 * @returns {Promise<Object>}
 */
export async function executeBatchExperiences({
  ...
  // Utiliser le timestamp partagé ou en générer un nouveau
  const generationTimestamp = debugTimestamp || generateTimestamp();
