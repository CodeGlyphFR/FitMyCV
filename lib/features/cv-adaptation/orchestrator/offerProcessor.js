 ...
 */
export async function runBatchesPhase(offer, sourceCv, classificationResult, jobOffer, targetLanguage, userId, signal, progressContext) {
  ...
  // 1. Parallelize experiences, projects, extras
 ...
 */
export async function processOffer(offer, sourceCv, sourceCvFileName, jobOffer, jobOfferId, targetLanguage, userId, signal, progressContext, hasAnalyticsConsent = false) {
  ...
  try {
    ...
    // Telemetry: CV_GENERATION_V2_COMPLETED (Story 5.3)
    ...
    // Telemetry: CV_GENERATION_V2_FAILED (Story 5.3)
    if (hasAnalyticsConsent) {
