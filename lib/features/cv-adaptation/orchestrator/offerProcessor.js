 ...
 */
export async function runBatchesPhase(offer, sourceCv, classificationResult, jobOffer, sourceLanguage, targetLanguage, userId, signal, progressContext) {
  ...
  // 1. Parallelize experiences, projects, extras, education, languages
  console.log(`[orchestrator] Running parallel batches: experiences, projects, extras, education, languages`);
  ...
  const [experiencesResult, projectsResult, extrasResult, educationResult, languagesResult] = await Promise.all([
    // Experiences
    withRetry(
      async () => {
  ...
  // 2. Skills and Summary in parallel (same Cache B: Job Offer + Experiences + Projects)
  console.log(`[orchestrator] Running parallel batches: skills, summary`);

  // Emit progress: skills and summary in progress
  ...
  const [skillsResult, summaryResult] = await Promise.all([
    ...
    // Summary (no longer depends on skills, uses Cache B)
    withRetry(
      async () => {
 ...
 */
export async function processOffer(offer, sourceCv, sourceCvFileName, jobOffer, jobOfferId, sourceLanguage, targetLanguage, userId, signal, progressContext, hasAnalyticsConsent = false) {
