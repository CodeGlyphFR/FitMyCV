...
import { normalizeSection } from '@/lib/cv-core/normalizeSection.js';
...
import { withRetry, MAX_RETRIES, NonRetryableError, isNonRetryableError } from './retryHandler.js';
import { emitProgress } from './progressEmitter.js';
import { generateTimestamp } from '../utils/debugLog.js';

/**
 * Execute Phase 0: Extraction of the job offer
 */
export async function runExtractionPhase(offer, userId, signal, progressContext, { markdownContent, markdownTitle } = {}) {
  console.log(`[orchestrator] Phase 0: Extraction for offer ${offer.id} from ${markdownContent ? 'markdown' : offer.sourceUrl}`);
  ...
  const result = await withRetry(
    async () => {
      const r = await executeExtraction({
        offerId: offer.id,
      ...
      if (!r.success) {
        if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
...
export async function runClassificationPhase(offer, sourceCv, jobOffer, userId, signal, progressContext) {
  ...
  const result = await withRetry(
    async () => {
      const r = await executeClassification({
        offerId: offer.id,
        sourceCv,
        jobOffer,
        userId,
...
export async function runBatchesPhase(offer, sourceCv, classificationResult, jobOffer, sourceLanguage, targetLanguage, userInterfaceLanguage, userId, signal, progressContext) {
  ...
  // Generate a shared timestamp for all debug logs of this generation
  // This ensures all logs (experiences, skills, summary) are in the same folder
  const debugTimestamp = generateTimestamp();
  ...
  const [experiencesResult, projectsResult, extrasResult, educationResult, languagesResult, skillsResult] = await Promise.all([
    ...
    withRetry(
      async () => {
        const r = await executeBatchExperiences({
          offerId: offer.id,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Experiences failed');
        }
        return r;
      },
      MAX_RETRIES,
    ...
    withRetry(
      async () => {
        const r = await executeBatchProjects({
          offerId: offer.id,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Projects failed');
        }
        return r;
      },
      MAX_RETRIES,
    ...
    withRetry(
      async () => {
        const r = await executeBatchExtras({
          offerId: offer.id,
          extras: normalizeSection(sourceCv.extras),
          jobOffer,
          targetLanguage,
          userId,
          signal,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Extras failed');
        }
        return r;
      },
      MAX_RETRIES,
    ...
    withRetry(
      async () => {
        const r = await executeBatchEducation({
          offerId: offer.id,
          education: normalizeSection(sourceCv.education),
          jobOffer,
          targetLanguage,
          userId,
          signal,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Education failed');
        }
        return r;
      },
      MAX_RETRIES,
    ...
    withRetry(
      async () => {
        const r = await executeBatchLanguages({
          offerId: offer.id,
          languages: normalizeSection(sourceCv.languages),
          jobOffer,
          sourceLanguage,
          targetLanguage,
          userId,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Languages failed');
        }
        return r;
      },
      MAX_RETRIES,
    ...
    withRetry(
      async () => {
        const r = await executeBatchSkills({
          offerId: offer.id,
        ...
        if (!r.success) {
          if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
          throw new Error(r.error || 'Batch Skills failed');
        }
        return r;
      },
      MAX_RETRIES,
  ...
  const summaryResult = await withRetry(
    async () => {
      const r = await executeBatchSummary({
        offerId: offer.id,
        ...
        adaptedSkills: processedSkills, // Passer les skills pour extraire les skills pertinents
      ...
      if (!r.success) {
        if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
        throw new Error(r.error || 'Batch Summary failed');
      }
      return r;
    },
    MAX_RETRIES,
...
export async function runRecompositionPhase(offer, sourceCv, sourceCvFileName, batchResults, jobOffer, jobOfferId, targetLanguage, userId, signal, progressContext) {
  ...
  const result = await withRetry(
    async () => {
      const r = await executeRecomposition({
        offerId: offer.id,
        sourceCv,
        sourceCvFileName,
        batchResults,
      ...
      if (!r.success) {
        if (isNonRetryableError(r.error)) throw new NonRetryableError(r.error);
        throw new Error(r.error || 'Recomposition failed');
      }
      return r;
    },
    MAX_RETRIES,
