...
import { trackCvGenerationV2Started } from '@/lib/telemetry/server';
    ...
    if (currentBgTask?.status !== 'cancelled') {
      await prisma.backgroundTask.update({
        ...
        data: {
          ...
          successMessage: offerResult.success ? 'CV generated successfully' : null,
        },
      }).catch(() => {});
    }
...
export async function startMultiOfferGeneration(taskId) {
  ...
  try {
    ...
    // Telemetry: CV_GENERATION_V2_STARTED (Story 5.3)
      ...
      await prisma.backgroundTask.update({
        ...
        data: {
          ...
          successMessage: results.completed.length > 0
            ? `${results.completed.length} CV(s) generated successfully`
            : null,
        },
      }).catch(() => {});
    }
