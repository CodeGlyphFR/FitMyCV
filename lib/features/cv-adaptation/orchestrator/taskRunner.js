...
export async function startSingleOfferGeneration(taskId, offerId) {
  ...
  try {
    ...
    // Retrieve payload from BackgroundTask
    ...
    try {
      ...
      // Fallback to defaults if payload parsing fails
    }
    ...
    if (!jobOfferId && (offer.sourceUrl || markdownContent)) {
      try {
        const extractionResult = await runExtractionPhase(offer, userId, signal, progressContext, { markdownContent, markdownTitle });
        ...
        // Refund credit (sauf si annulé — l'API cancel gère le remboursement)
        const refundResult = isCancelled
          ? { success: false, amount: 0 }
          : await refundCreditForOffer(taskId, offer.id, userId, errorMessage);
    ...
    if (currentBgTask?.status !== 'cancelled') {
      await prisma.backgroundTask.update({
        ...
        data: {
          ...
          ...(offerResult.success && offerResult.filename && { cvFile: offerResult.filename }),
        },
      }).catch(() => {});
    }

    // If failed, refund credit (sauf si annulé — l'API cancel gère le remboursement)
    let creditsRefunded = 0;
    const isCancelledResult = signal?.aborted || offerResult.cancelled;
    if (!offerResult.success && !isCancelledResult) {
    ...
    if (offer?.id && userId && !isCancelled) {
      refundResult = await refundCreditForOffer(taskId, offer.id, userId, error.message);
    }
    ...
    // Emit SSE events (only if we have a valid user)
    if (userId) {
      dbEmitter.emitCvGenerationOfferFailed(userId, {
        taskId,
...
export async function startMultiOfferGeneration(taskId) {
  ...
  try {
    ...
    // Don't overwrite if already 'cancelled' by API
    if (currentBgTask?.status !== 'cancelled') {
      const firstGeneratedCv = results.completed[0]?.filename || null;
      await prisma.backgroundTask.update({
        where: { id: taskId },
        data: {
          status: finalStatus,
          ...
          successMessage: results.completed.length > 0
            ...
            : null,
          ...(firstGeneratedCv && { cvFile: firstGeneratedCv }),
        },
      }).catch(() => {});
    }
