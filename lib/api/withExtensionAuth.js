...
export function withExtensionAuth(handler) {
  return async function authenticatedHandler(request, routeContext) {
    ...
    // M5: Vérifier que le tokenVersion correspond (révocation après changement de mot de passe)
    if (tokenPayload.tokenVersion !== user.tokenVersion) {
      console.warn(`[extension-auth] Token revoked (tokenVersion mismatch) for user: ${user.id}`);
