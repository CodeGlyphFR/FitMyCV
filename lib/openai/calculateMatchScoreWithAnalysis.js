...
export async function calculateMatchScoreWithAnalysis({
  ...
  try {
    ...
    // Logique incrémentale : si un score existe déjà, ajouter +0 à +3 aléatoirement
    let finalMatchScore;
    let finalScoreBreakdown;

    if (previousScore !== null && previousScore !== undefined) {
      // Score existe déjà → ajouter +0 à +3 aléatoirement
      const randomBonus = Math.floor(Math.random() * 4); // 0, 1, 2 ou 3
      finalMatchScore = Math.min(100, previousScore + randomBonus);

      // Ajuster le breakdown pour correspondre au nouveau score
      finalScoreBreakdown = adjustScoreBreakdown(gptScoreBreakdown, finalMatchScore);

      console.log(`[calculateMatchScoreWithAnalysis] Score incrémental: ${previousScore} → ${finalMatchScore} (+${randomBonus} pts)`);
    } else {
