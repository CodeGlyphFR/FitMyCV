 ...
 */
async function callChatGPTWithVision(client, model, imageBase64Array, visionDetail = 'high', detectedLanguage = 'fr', signal = null, temperature = 0.1, topP = 1, seed = 0) {
  try {
...
export async function importPdfCv({ pdfFilePath, userId = null, isFirstImport = false, signal = null }) {
  ...
  // 5bis. Detecter la langue du PDF AVANT extraction (utilise model_detect_language + pdf_vision_detail)
  console.log(`[importPdf] Détection de la langue du PDF (detail: ${pdfConfig.detail})...`);
  ...
  const result = await callChatGPTWithVision(client, model, imageBase64Array, pdfConfig.detail, detectedLanguage, signal, pdfConfig.temperature, pdfConfig.topP, pdfConfig.seed);
