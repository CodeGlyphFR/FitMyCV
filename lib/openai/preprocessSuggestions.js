...
/**
 * Analyse les suggestions cochées et identifie les cibles (expérience/projet)
 ...
 * @returns {Promise<Object>} - { experienceImprovements, projectImprovements, newProjects, unmatchedSuggestions, success }
 */
export async function preprocessSuggestions({
  ...
  // Charger les prompts
  const systemPrompt = await loadPromptWithVars('lib/openai/prompts/improve-cv-v2/preprocess-system.md', {});
  const userPrompt = await loadPromptWithVars('lib/openai/prompts/improve-cv-v2/preprocess-user.md', {
    ...
    if (userId && response.usage) {
      try {
        await trackOpenAIUsage({
          userId,
          featureName: 'cv_improvement_v2_preprocess',
          model,
