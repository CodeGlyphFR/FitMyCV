...
export async function generateCvWithScore({
  ...
  console.log('[generateCvWithScore] Vérification des crédits OpenAI...');
  try {
    await checkOpenAICredits();
    console.log('[generateCvWithScore] ✅ Crédits OpenAI disponibles');
  } catch (error) {
    console.error('[generateCvWithScore] ❌ Erreur crédits OpenAI:', error.message);
    throw error;
  }
