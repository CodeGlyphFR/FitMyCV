...
/**
 * Nettoie le HTML en supprimant les éléments inutiles pour réduire la taille
...
function cleanHtml(html) {
  ...
  // Supprimer les attributs inutiles (onclick, onload, data-*, style, class sur certaines balises)
  cleaned = cleaned.replace(/\s+(onclick|onload|onmouseover|onmouseout|onfocus|onblur)="[^"]*"/gi, '');
  cleaned = cleaned.replace(/\s+data-[a-z-]+="[^"]*"/gi, '');
  cleaned = cleaned.replace(/\s+style="[^"]*"/gi, '');

  // Réduire les espaces multiples et les retours à la ligne excessifs
...
async function extractJobOfferWithGPT(url, userId = null) {
  ...
  try {
    ...
    try {
      ...
      // Essayer d'extraire directement le contenu avec des sélecteurs CSS Indeed
      console.log('[createTemplateCv - extractJobOfferWithGPT] Extraction ciblée du contenu Indeed...');
      const extracted = await page.evaluate(() => {
        // Sélecteurs CSS pour Indeed
      ...
      if (extracted.description && extracted.description.length > 100) {
        console.log(`[createTemplateCv - extractJobOfferWithGPT] ✅ Extraction ciblée réussie (${extracted.description.length} caractères)`);
        extractedContent = extracted.description;
        jobTitle = extracted.title;
      } else {
        console.log('[createTemplateCv - extractJobOfferWithGPT] ⚠️ Extraction ciblée insuffisante, récupération du HTML complet...');
        extractedContent = await page.content();
        console.log(`[createTemplateCv - extractJobOfferWithGPT] HTML complet récupéré (${extractedContent.length} caractères)`);
      }
    ...
    console.log('[createTemplateCv - extractJobOfferWithGPT] Nettoyage du HTML...');
    const cleanedHtml = cleanHtml(extractedContent);
    console.log(`[createTemplateCv - extractJobOfferWithGPT] HTML nettoyé: ${extractedContent.length} → ${cleanedHtml.length} caractères`);

    // Appliquer une limite stricte de 60000 caractères (~15k tokens)
    const MAX_HTML_CHARS = 60000;
    let finalHtml = cleanedHtml;

    if (cleanedHtml.length > MAX_HTML_CHARS) {
      console.log(`[createTemplateCv - extractJobOfferWithGPT] ⚠️ HTML trop volumineux (${cleanedHtml.length} chars), troncature à ${MAX_HTML_CHARS} chars...`);
      finalHtml = cleanedHtml.substring(0, MAX_HTML_CHARS) + '\n\n[... Contenu tronqué pour respecter les limites ...]';
    }

    console.log(`[createTemplateCv - extractJobOfferWithGPT] HTML final à envoyer à GPT: ${finalHtml.length} caractères`);
...
${finalHtml}`;
    ...
    // Utiliser le titre extrait directement, sinon chercher dans le texte GPT
    let finalTitle = jobTitle;
    if (!finalTitle) {
      const jobTitleMatch = text.match(/📋 TITRE DU POSTE:\s*\n(.+)/i);
      finalTitle = jobTitleMatch ? jobTitleMatch[1].trim() : 'Offre d\'emploi';
    }

    console.log(`[createTemplateCv - extractJobOfferWithGPT] Titre de l'offre: ${finalTitle}`);
