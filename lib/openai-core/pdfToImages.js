...
export async function getPdfImageConfig() {
  ...
  try {
    ...
    cachedPdfConfig = {
      ...
      temperature: parseFloat(getSettingValue('temperature_import_pdf', String(PDF_CONVERSION_CONFIG.TEMPERATURE))),
      topP: parseFloat(getSettingValue('top_p_import_pdf', String(PDF_CONVERSION_CONFIG.TOP_P))),
      seed: parseInt(getSettingValue('seed_import_pdf', String(PDF_CONVERSION_CONFIG.SEED)), 10),
    };
  } catch (error) {
    ...
    cachedPdfConfig = {
      ...
      temperature: PDF_CONVERSION_CONFIG.TEMPERATURE,
      topP: PDF_CONVERSION_CONFIG.TOP_P,
      seed: PDF_CONVERSION_CONFIG.SEED,
    };
  }
  ...
  try {
    ...
    const base64Images = await Promise.all(
      results.map(async (result, index) => {
        ...
        try {
          // Resize PUIS trim = 4x moins de pixels à analyser
          // (trim sur 1000px au lieu de 2000px)
          ...
          console.log(`[pdfToImages] Page ${index + 1}: ${Math.round(resizedBuffer.length / 1024)}KB (trimmed)`);

          return base64;
        } catch (sharpError) {
          // Fallback sans trim si erreur (page blanche, etc.)
          console.warn(`[pdfToImages] Page ${index + 1}: trim failed, using untrimmed`);
          try {
            const resizedBuffer = await sharp(result.buffer)
              .resize({
                width: maxWidth,
            ...
            console.error(`[pdfToImages] Erreur sharp page ${index + 1}:`, fallbackError.message);
