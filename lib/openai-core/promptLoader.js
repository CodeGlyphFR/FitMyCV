...
const SHARED_PROMPTS_DIR = path.join(process.cwd(), 'lib/prompts-shared');
...
export async function loadPrompt(promptPath, useCache = process.env.NODE_ENV === 'production', processIncludes = true) {
  ...
  // Si le chemin commence par 'lib/.../', utiliser depuis cwd
  // Sinon, utiliser PROMPTS_DIR
  let fullPath;
  if (promptPath.startsWith('lib/openai/prompts/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else if (promptPath.startsWith('lib/prompts-shared/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else if (promptPath.startsWith('lib/features/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else if (promptPath.startsWith('lib/job-offer/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else if (promptPath.startsWith('lib/scoring/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else if (promptPath.startsWith('lib/translation/')) {
    fullPath = path.join(process.cwd(), promptPath);
  } else {
...
async function processIncludeDirectives(content, useCache) {
  ...
  for (const include of includes) {
    try {
      // Résoudre le chemin de l'include
      // Support pour _shared/ -> lib/prompts-shared/
      let includePath = include.path;
      if (includePath.startsWith('_shared/')) {
        includePath = `lib/prompts-shared/${includePath.replace('_shared/', '')}`;
      }

      // Charger le fichier inclus (récursivement pour supporter les includes imbriqués)
      const includedContent = await loadPrompt(includePath, useCache, true);
      content = content.replace(include.fullMatch, includedContent);
      console.log(`[promptLoader] 📎 Included: ${includePath}`);
    } catch (error) {
...
export async function loadPromptWithVars(promptPath, variables = {}) {
  ...
  // Remplacer les placeholders {{variable}} et {variable}
  for (const [key, value] of Object.entries(variables)) {
    // Convertir la valeur en string (gérer les objets, null, undefined)
    ...
    // Support {{variable}} (double curly braces - style Handlebars/Mustache)
    const doublePlaceholder = `{{${key}}}`;
    prompt = prompt.replace(new RegExp(doublePlaceholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), strValue);

    // Support {variable} (single curly braces - legacy)
    const singlePlaceholder = `{${key}}`;
    prompt = prompt.replace(new RegExp(singlePlaceholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), strValue);
  }
