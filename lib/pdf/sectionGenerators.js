                ...
                <div class="experience-header-block${headerBreak}">
                  <div class="experience-header">
                    <div>
                      ...
                      ${exp.company ? `<div class="experience-company">${exp.company}${exp.department_or_client ? ` (${exp.department_or_client})` : ''}${exp.location ? `<span class="experience-location"> - ${formatLocation(exp.location)}</span>` : ''}${exp.url ? `<a href="${exp.url}" class="experience-link" target="_blank"><svg class="link-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>${exp.url_label || exp.url}</a>` : ''}</div>` : ''}
                    </div>
                    ${exp.start_date || exp.end_date ? `<div class="experience-dates">${formatDate(exp.start_date, language)} – ${formatDate(exp.end_date, language) || getTranslation(language, "cvSections.present")}</div>` : ''}
                  </div>
            ...
            return `
              <div class="project-item${itemBreak}">
                <div class="project-header">
                  <div>
                    ${project.name ? `<div class="project-name">${project.name}${project.url ? ` <a href="${project.url}" class="project-link" target="_blank"><svg class="link-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>${project.url_label || project.url}</a>` : ''}</div>` : ''}
