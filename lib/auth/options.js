...
export const authOptions = {
  ...
  callbacks: {
    ...
    async signIn({ user, account }){
      ...
      try {
        ...
        await saveLoginFingerprint(user.id, fpCookie?.value || null, reqLike);
      } catch (fpError) {
  ...
  events: {
    ...
    async createUser({ user }){
      ...
      try {
        await assignDefaultPlan(user.id, null, { skipWelcomeCredits: fpResult.isDuplicate });
        logger.context('auth', 'info', `Plan gratuit attribué à user ${user.id}${fpResult.isDuplicate ? ' (crédits bienvenue bloqués - fingerprint dupliqué)' : ''}`);
      } catch (error) {
