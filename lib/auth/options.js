...
async function ensureUserWorkspace(user){
  ...
  const files = await listUserCvFiles(user.id).catch(() => []);
  const hasMain = files.includes("main.json");
  ...
  if (!hasMain){
    ...
    await writeUserCvFile(user.id, "main.json", JSON.stringify(payload, null, 2));
  }
  try {
    await prisma.cvFile.upsert({
      where: { userId_filename: { userId: user.id, filename: "main.json" } },
      update: {},
      create: { userId: user.id, filename: "main.json" },
    });
  } catch (error) {
    console.error("Impossible d'enregistrer le CV principal", error);
  }
}
