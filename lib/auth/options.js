...
import { writeUserCvFile, listUserCvFiles } from "@/lib/cv-core/storage";
...
import { checkAndSaveFingerprint, saveLoginFingerprint } from "@/lib/fingerprint/fingerprintService";
...
// Protection brute-force pour le login web (par email)
...
export const authOptions = {
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials){
        ...
        if (attemptRecord && attemptRecord.count >= MAX_LOGIN_ATTEMPTS) {
          const elapsed = Date.now() - attemptRecord.lastAttempt;
          if (elapsed < LOGIN_LOCKOUT_DURATION) {
            const remainingMin = Math.ceil((LOGIN_LOCKOUT_DURATION - elapsed) / 60000);
            logger.context('auth', 'warn', `Login bloqué (brute-force lockout) pour: ${email}, reste ${remainingMin}min`);
  ...
  callbacks: {
    async jwt({ token, user, trigger, account }){
      ...
      if (token?.id) {
        const dbUser = await prisma.user.findUnique({
          where: { id: token.id },
          select: { emailVerified: true, passwordHash: true, role: true, tokenVersion: true },
        });
        ...
        // H2/M5: Vérifier que le tokenVersion correspond (invalidation après changement de mot de passe)
        if ((token.tokenVersion ?? 0) !== (dbUser.tokenVersion ?? 0)) {
          token.invalid = true;
          return token;
        }
    ...
    async session({ session, token }){
      // Si le token est marqué comme invalide (utilisateur supprimé ou tokenVersion changé), invalider
      if (token?.invalid) {
        ...
        if (!lastLogTime || (now - lastLogTime) > INVALID_SESSION_LOG_THROTTLE) {
          logger.context('session', 'warn', `User ${token.id} session invalidée (token révoqué ou utilisateur supprimé)`);
    ...
    async signIn({ user, account }){
      ...
      // Pour un nouveau user OAuth, le user n'existe pas encore en base
      // (signIn est appelé AVANT createUser par NextAuth).
      // Le createUser event gère tout (plan, fingerprint, tracking, lastLoginAt).
      const isNewOAuthUser = account && !dbUser;
      ...
      if (!isNewOAuthUser) {
        ...
        try {
          const result = await assignDefaultPlan(resolvedUserId);
          if (result.success && result.subscription) {
            logger.context('auth', 'info', `Plan par défaut attribué (signIn fallback) à user ${resolvedUserId}`);
          }
        } catch (error) {
          console.error('[auth] Erreur attribution plan par défaut (signIn fallback):', error);
        }

        // Sauvegarder/rafraîchir le fingerprint navigateur au login
        try {
          const cookieStore = await cookies();
          const fpCookie = cookieStore.get('fitmycv_fp');
          const headerStore = await headers();
          // Construire un objet request-like pour extraire l'IP
          const reqLike = { headers: { get: (name) => headerStore.get(name) } };
          await saveLoginFingerprint(resolvedUserId, fpCookie?.value || null, reqLike);
        } catch (fpError) {
          console.error('[auth] Erreur sauvegarde fingerprint (login):', fpError);
        }

        // Mettre à jour lastLoginAt pour le suivi d'inactivité (RGPD)
        try {
          await prisma.user.update({
          ...
          console.error('[auth] Erreur mise à jour lastLoginAt:', updateError);
        }
  ...
  events: {
    async signOut({ token }) {
      // Terminer la session de tracking et envoyer la notification Telegram
      if (token?.sub) {
        try {
          const { endSessionsByUserId } = await import('@/lib/session/sessionManager');
          await endSessionsByUserId(token.sub);
        } catch (err) {
          console.error('[auth] Erreur fin de session Telegram:', err);
        }
      }
    },
    async createUser({ user }){
      ...
      const isOAuthUser = dbUser && !dbUser.passwordHash;
      if (isOAuthUser && !dbUser.emailVerified) {
        updateData.emailVerified = new Date();
        trackServerEvent(user.id, 'email_verified', { method: 'oauth' });
      ...
      // Envoyer l'email de bienvenue aux utilisateurs OAuth
      // (Les utilisateurs credentials reçoivent l'email après vérification de leur email)
      if (isOAuthUser) {
        try {
          ...
          logger.context('auth', 'info', `Email de bienvenue envoyé à OAuth user ${user.id}`);
        } catch (emailError) {
          // Ne pas bloquer la création de compte si l'email échoue
          logger.context('auth', 'error', `Échec envoi email de bienvenue OAuth user ${user.id}: ${emailError.message}`);
        }
      }
      ...
      // Vérifier le fingerprint navigateur + IP (anti-abus multi-compte)
      ...
      try {
        ...
        const headerStore = await headers();
        const reqLike = { headers: { get: (name) => headerStore.get(name) } };
        fpResult = await checkAndSaveFingerprint(user.id, fpCookie?.value || null, reqLike);
      } catch (fpError) {
        console.error('[auth] Erreur fingerprint check (OAuth):', fpError);
      ...
      try {
        ...
        await assignDefaultPlan(user.id, null, { skipWelcomeCredits });
        const fpNote = fpResult.isDuplicate
          ...
          : '';
        logger.context('auth', 'info', `Plan gratuit attribué à user ${user.id}${fpNote}`);
      } catch (error) {
