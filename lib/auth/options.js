  ...
  callbacks: {
    ...
    async signIn({ user, account }){
      ...
      if (account) {
        ...
        // OAuth user existant ou nouveau autorisé - assigner le plan gratuit comme fallback
        if (dbUser && !dbUser.passwordHash) {
          try {
            const result = await assignDefaultPlan(dbUser.id);
            if (result.success) {
              logger.context('auth', 'info', `Plan gratuit attribué (signIn fallback) à user ${dbUser.id}`);
            }
          } catch (error) {
