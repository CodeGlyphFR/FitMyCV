...
function isModificationAccepted(decisions, section, index, field) {
  ...
  // Debug logging pour identifier les mismatches de clés
  console.log(`[applySelectiveChanges] Checking key: "${key}" -> decision: ${decision || 'NOT_FOUND'}`);
  if (decision === undefined) {
    // Log les clés disponibles pour cette section pour debug
    const sectionKeys = Object.keys(decisions).filter(k => k.startsWith(section + ':'));
    console.log(`[applySelectiveChanges] Available keys for ${section}:`, sectionKeys);
  }
...
function applyExperienceChanges(sourceExperiences, batchExperiences, decisions) {
  ...
  return batchExperiences.map((adaptedExp, expIndex) => {
    ...
    modifications.forEach((mod, modIndex) => {
      ...
      if (!isAccepted) {
        // Restaurer la valeur originale - utiliser mod.before en priorité
        const field = mod.field.toLowerCase();
        console.log(`[applySelectiveChanges] REJECTED: Restoring ${field} for exp "${adaptedExp.title}"`);
        console.log(`[applySelectiveChanges]   mod.before: "${mod.before}", mod.after: "${mod.after}"`);

        if (field === 'title') {
          if (mod.before !== undefined) {
            console.log(`[applySelectiveChanges]   -> Restoring title from mod.before: "${mod.before}"`);
            finalExp.title = mod.before;
          } else if (sourceExp.title !== undefined) {
            console.log(`[applySelectiveChanges]   -> Restoring title from sourceExp: "${sourceExp.title}"`);
            finalExp.title = sourceExp.title;
          } else {
            console.log(`[applySelectiveChanges]   -> WARNING: No value to restore for title!`);
          }
        } else if (field === 'description') {
          if (mod.before !== undefined) {
            finalExp.description = mod.before;
          } else if (sourceExp.description !== undefined) {
            finalExp.description = sourceExp.description;
          }
        } else if (field === 'responsibilities') {
          if (mod.before !== undefined) {
            finalExp.responsibilities = Array.isArray(mod.before) ? mod.before : [mod.before];
          } else if (sourceExp.responsibilities !== undefined) {
            finalExp.responsibilities = sourceExp.responsibilities;
          }
        } else if (field === 'deliverables') {
          if (mod.before !== undefined) {
            finalExp.deliverables = Array.isArray(mod.before) ? mod.before : [mod.before];
          } else if (sourceExp.deliverables !== undefined) {
            finalExp.deliverables = sourceExp.deliverables;
          }
        } else if (field === 'skills_used') {
          if (mod.before !== undefined) {
            finalExp.skills_used = Array.isArray(mod.before) ? mod.before : [mod.before];
          } else if (sourceExp.skills_used !== undefined) {
  ...
  return batchProjects.map((adaptedProj, projIndex) => {
    ...
    modifications.forEach((mod, modIndex) => {
      ...
      if (!isAccepted) {
        ...
        if (field === 'summary') {
          if (mod.before !== undefined) {
            finalProj.summary = mod.before;
          } else if (sourceProj.summary !== undefined) {
            finalProj.summary = sourceProj.summary;
          }
        } else if (field === 'tech_stack') {
          if (mod.before !== undefined) {
            finalProj.tech_stack = Array.isArray(mod.before) ? mod.before : [mod.before];
          } else if (sourceProj.tech_stack !== undefined) {
            finalProj.tech_stack = sourceProj.tech_stack;
          }
        } else if (field === 'description') {
          if (mod.before !== undefined) {
            finalProj.description = mod.before;
          } else if (sourceProj.description !== undefined) {
    ...
    if (!isAccepted) {
      ...
      switch (mod.action) {
        ...
        case 'removed':
          ...
          // Utiliser mod.before en priorité, fallback sur sourceSkills
          if (category === 'hard_skills') {
            if (mod.before !== undefined) {
              finalSkills.hard_skills.push(
                typeof mod.before === 'object' ? mod.before : { name: skillName, level: mod.before }
              );
            } else {
            ...
            if (mod.before !== undefined) {
              finalSkills.soft_skills.push(mod.before);
            } else if (sourceSkills?.soft_skills?.includes(skillName)) {
              finalSkills.soft_skills.push(skillName);
            }
          } else if (category === 'tools') {
            if (mod.before !== undefined) {
              finalSkills.tools.push(
                typeof mod.before === 'object' ? mod.before : { name: skillName, level: mod.before }
              );
            } else {
            ...
            if (mod.before !== undefined) {
              finalSkills.methodologies.push(mod.before);
            } else if (sourceSkills?.methodologies?.includes(skillName)) {
...
function applySummaryChanges(sourceSummary, batchSummary, decisions) {
  ...
  console.log(`[applySelectiveChanges] Summary has ${modifications.length} modification(s)`);
  ...
  modifications.forEach((mod, modIndex) => {
    ...
    if (!isAccepted) {
      const field = mod.field.toLowerCase();
      console.log(`[applySelectiveChanges] REJECTED summary ${field}`);
      console.log(`[applySelectiveChanges]   mod.before: "${mod.before}", mod.after: "${mod.after}"`);

      // Utiliser mod.before en priorité, fallback sur sourceSummary
      if (field === 'description') {
        if (mod.before !== undefined) {
          console.log(`[applySelectiveChanges]   -> Restoring description from mod.before`);
          finalSummary.description = mod.before || '';
        } else if (sourceSummary?.description !== undefined) {
          console.log(`[applySelectiveChanges]   -> Restoring description from sourceSummary`);
          finalSummary.description = sourceSummary.description;
        } else {
          console.log(`[applySelectiveChanges]   -> WARNING: No value to restore for description!`);
        }
      } else if (field === 'domains') {
        if (mod.before !== undefined) {
          // mod.before peut être un array ou une string
          finalSummary.domains = Array.isArray(mod.before)
            ? mod.before
            : mod.before
              ? mod.before.split(',').map((d) => d.trim())
              : [];
        } else if (sourceSummary?.domains !== undefined) {
          finalSummary.domains = sourceSummary.domains;
        }
      } else if (field === 'key_strengths') {
        if (mod.before !== undefined) {
          finalSummary.key_strengths = Array.isArray(mod.before)
            ? mod.before
            : mod.before
              ? [mod.before]
              : [];
        } else if (sourceSummary?.key_strengths !== undefined) {
...
function applyExtrasChanges(sourceExtras, batchExtras, decisions) {
  ...
  if (Array.isArray(batchExtras)) {
    return batchExtras.map((adaptedExtra, extraIndex) => {
      ...
      modifications.forEach((mod, modIndex) => {
        ...
        if (!isAccepted) {
          // Utiliser mod.before en priorité, fallback sur sourceExtra
          if (mod.before !== undefined) {
            finalExtra[mod.field] = mod.before;
          } else if (sourceExtra[mod.field] !== undefined) {
  ...
  if (batchExtras.modifications) {
    ...
    batchExtras.modifications.forEach((mod, modIndex) => {
      ...
      if (!isAccepted) {
        // Utiliser mod.before en priorité, fallback sur sourceExtras
        if (mod.before !== undefined) {
          finalExtras[mod.field] = mod.before;
        } else if (sourceExtras?.[mod.field] !== undefined) {
...
function applyLanguagesChanges(sourceLanguages, batchLanguages, decisions) {
  ...
  // Support pour les détails de modification avec before/after
  const modificationDetails = batchLanguages.modifications.details || [];
  ...
  return adaptedLanguages.map((lang, index) => {
    ...
    if (wasAdapted) {
      ...
      if (!isAccepted) {
        // Chercher d'abord dans les détails de modification pour mod.before
        const modDetail = modificationDetails.find(
          (d) => (d.name || '').toLowerCase() === langName.toLowerCase()
        );

        if (modDetail?.before !== undefined) {
          // Utiliser mod.before
          return modDetail.before;
        }

        // Fallback: restaurer la langue originale depuis source
