...
export async function executeClassification({
  ...
  try {
    ...
    const { classification, model, promptTokens, completionTokens, duration: coreDuration } = result;
    const totalDuration = Date.now() - startTime;

    // 3. Tracker l'usage OpenAI
    if (userId) {
    ...
    // 4. Sanitize classification for PostgreSQL (remove \u0000 characters)
    const sanitizedClassification = sanitizeForPostgres(classification);

    // 5. Mettre a jour la subtask avec succes
