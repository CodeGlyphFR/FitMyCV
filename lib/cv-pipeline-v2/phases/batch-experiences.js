 ...
 * @param {Object} params.jobOfferInfo - Infos extraites de l'offre
 * @param {string} params.targetLanguage - Langue cible
 * @param {string} params.model - Modele IA a utiliser
 * @param {Object} params.systemPrompt - Prompt systeme
...
async function adaptSingleExperience({
  ...
  try {
    // Preparer le prompt utilisateur
    const userPrompt = replaceVariables(userPromptTemplate, {
      experienceIndex: String(experienceIndex),
    ...
    // Mettre a jour la subtask
    await prisma.cvGenerationSubtask.update({
      where: { id: subtask.id },
      data: {
        status: 'completed',
        output: adaptedExperience,
