...
import { initializeReviewState, generateChangeId } from '@/lib/cv/changeTracking';
...
function extractChangesFromBatchResults(batchResults, sourceCv) {
  ...
  if (batchResults.summary?.modifications && Array.isArray(batchResults.summary.modifications)) {
    for (const mod of batchResults.summary.modifications) {
      ...
      if (mod.field === 'domains' && mod.action === 'generated') {
        const beforeValue = sourceCv.summary?.domains || [];
        const afterValue = batchResults.summary?.domains || [];
        ...
        if (arraysAreDifferent(beforeValue, afterValue)) {
          changes.push({
            ...
            section: 'summary',
            field: 'domains',
            path: 'summary.domains',
            changeType: 'modified',
            beforeValue,
            afterValue,
            beforeDisplay: beforeValue.join(', '),
            afterDisplay: afterValue.join(', '),
            change: 'Domaines d\'expertise adaptes',
            reason: mod.reason || 'Adaptation au poste cible',
            status: 'pending',
          });
        }
