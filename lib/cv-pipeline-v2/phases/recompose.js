 ...
 * et les convertit au format attendu par le systeme de review
 */
    ...
    for (const [expIndexStr, expMods] of Object.entries(batchResults.experienceModifications)) {
      ...
      if (expMods.modifications && Array.isArray(expMods.modifications)) {
        for (const mod of expMods.modifications) {
          if (mod.action === 'modified' && mod.field) {
            changes.push({
              id: generateChangeId(),
              section: 'experience',
              field: mod.field,
              ...
              expIndex,
              changeType: 'modified',
              itemName: mod.field === 'title' ? 'Titre' :
                       mod.field === 'description' ? 'Description' :
                       mod.field === 'responsibilities' ? 'Responsabilités' :
                       mod.field === 'deliverables' ? 'Résultats' : mod.field,
              beforeValue: mod.before,
              afterValue: mod.after,
              beforeDisplay: typeof mod.before === 'string' ? mod.before : JSON.stringify(mod.before),
              afterDisplay: typeof mod.after === 'string' ? mod.after : JSON.stringify(mod.after),
              change: `${mod.field} modifié`,
              reason: mod.reason || 'Adaptation au poste ciblé',
              status: 'pending',
            });
          }
        }
  ...
  if (batchResults.summaryModifications) {
    ...
    if (mods.description) {
      changes.push({
        ...
        changeType: 'modified',
        beforeValue: mods.description.before,
        afterValue: mods.description.after,
        beforeDisplay: mods.description.before || '',
        afterDisplay: mods.description.after || '',
        change: 'Description du profil adaptée',
        reason: mods.description.reason || 'Adaptation au poste ciblé',
        status: 'pending',
      });
    }
  }
...
export async function executeRecomposition({
  ...
  try {
    ...
    try {
      // D'abord, extraire les modifications de l'IA (avec les vraies raisons)
      ...
      // Ensuite, calculer les diffs programmatiques pour les autres changements (skills, etc.)
      const programmaticChanges = computeCvDiff(adaptedCv, sourceCv);
      console.log(`[recompose] computeCvDiff detected ${programmaticChanges.length} change(s)`);

      // Fusionner: utiliser les changements de l'IA en priorité, compléter avec les programmatiques
      // Pour éviter les doublons, on filtre les changements programmatiques qui sont déjà couverts par l'IA
      const aiChangeKeys = new Set(aiChanges.map(c => `${c.section}.${c.field}.${c.expIndex || ''}`));
      const additionalChanges = programmaticChanges.filter(c => {
        const key = `${c.section}.${c.field}.${c.expIndex || ''}`;
