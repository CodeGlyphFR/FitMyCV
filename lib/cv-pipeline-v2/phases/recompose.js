...
import { computeCvDiff, initializeReviewState, generateChangeId } from '@/lib/cv/changeTracking';
 ...
 *
 * IMPORTANT: Pour les champs array (responsibilities, deliverables, skills_used),
 * le schema IA force before/after comme strings (descriptions textuelles).
 * On ne cree donc PAS de changements pour ces champs ici - computeCvDiff le fera
 * avec les vraies valeurs bullet-level.
 *
 * On retourne:
 * - aiChanges: Changements pour title/description (avec vraies valeurs du CV)
 * - aiReasons: Map des raisons IA pour enrichir les changements programmatiques
 *
 * @returns {{ aiChanges: Array, aiReasons: Map<string, string> }}
 */
function extractChangesFromBatchResults(batchResults, sourceCv) {
  const changes = [];
  const aiReasons = new Map(); // Map<"exp.{expIndex}.{field}" | "summary.description", reason>

  // Extraire les modifications des experiences
  if (batchResults.experienceModifications) {
    for (const [expIndexStr, expMods] of Object.entries(batchResults.experienceModifications)) {
      const expIndex = parseInt(expIndexStr, 10);
      const originalExp = sourceCv.experience?.[expIndex];
      const adaptedExp = batchResults.experiences?.[expIndex];

      if (expMods.modifications && Array.isArray(expMods.modifications)) {
        for (const mod of expMods.modifications) {
          if (!mod.field) continue;

          // Stocker la raison IA pour enrichissement ulterieur (pour TOUS les champs)
          const reasonKey = `exp.${expIndex}.${mod.field}`;
          if (mod.reason) {
            aiReasons.set(reasonKey, mod.reason);
          }

          // Pour les champs TEXTE (title, description), creer un changement direct
          // avec les VRAIES valeurs du CV (pas les descriptions textuelles de l'IA)
          if (['title', 'description'].includes(mod.field) && mod.action === 'modified') {
            const beforeValue = originalExp?.[mod.field] || '';
            const afterValue = adaptedExp?.[mod.field] || '';

            // Ne creer le changement que si les valeurs sont reellement differentes
            if (beforeValue !== afterValue) {
              changes.push({
                ...
                expIndex,
                changeType: 'modified',
                itemName: mod.field === 'title' ? 'Titre' : 'Description',
                beforeValue,
                afterValue,
                beforeDisplay: beforeValue,  // VRAIE valeur!
                afterDisplay: afterValue,
                change: `${mod.field === 'title' ? 'Titre' : 'Description'} modifie`,
                reason: mod.reason || 'Adaptation au poste cible',
                status: 'pending',
              });
            }
          }
          // Pour les champs ARRAY (responsibilities, deliverables, skills_used):
          // NE PAS creer de changement ici - computeCvDiff le fera avec les vraies valeurs
          // La raison IA est deja stockee dans aiReasons pour enrichissement
        }
      }
    }
  }
  ...
  if (batchResults.summaryModifications) {
    const mods = batchResults.summaryModifications;
    if (mods.description) {
      // Recuperer les VRAIES valeurs du CV
      const beforeValue = sourceCv.summary?.description || '';
      const afterValue = batchResults.summary?.description || mods.description.after || '';
      ...
      if (beforeValue !== afterValue) {
        changes.push({
          ...
          reason: mods.description.reason || 'Adaptation au poste cible',
          status: 'pending',
        });
      }

      // Stocker aussi la raison pour enrichissement
      if (mods.description.reason) {
        aiReasons.set('summary.description', mods.description.reason);
      }
    }
  }
...
export async function executeRecomposition({
  ...
  try {
    ...
    // 6. Calculer les differences et initialiser le systeme de review
    try {
      // D'abord, extraire les modifications de l'IA
      // aiChanges: changements pour title/description (avec vraies valeurs)
      // aiReasons: Map des raisons IA pour enrichir les changements programmatiques
      const { aiChanges, aiReasons } = extractChangesFromBatchResults(batchResults, sourceCv);
      console.log(`[recompose] Extracted ${aiChanges.length} AI change(s), ${aiReasons.size} reason(s) from batchResults`);

      // Calculer les diffs programmatiques pour TOUS les changements
      // computeCvDiff detecte: skills (hard/soft/tools/methodologies), responsibilities, deliverables, skills_used, etc.
      const programmaticChanges = computeCvDiff(adaptedCv, sourceCv);
      console.log(`[recompose] computeCvDiff detected ${programmaticChanges.length} programmatic change(s)`);

      // Enrichir les changements programmatiques avec les raisons IA
      // Cela permet d'avoir les raisons explicatives de l'IA sur les changements bullet-level
      const enrichedProgrammaticChanges = programmaticChanges.map(c => {
        // Construire la cle pour chercher la raison IA
        let reasonKey = null;
        if (c.section === 'experience' && c.expIndex !== undefined) {
          reasonKey = `exp.${c.expIndex}.${c.field}`;
        } else if (c.section === 'summary') {
          reasonKey = `summary.${c.field}`;
        }

        // Si une raison IA existe pour ce champ, l'utiliser (plus explicative que la raison generique)
        if (reasonKey && aiReasons.has(reasonKey)) {
          return {
            ...c,
            reason: aiReasons.get(reasonKey),
          };
        }
        return c;
      });

      // Fusionner: AI changes (title/description) + programmatiques enrichis
      // Cle composite GRANULAIRE pour eviter les vrais doublons tout en gardant les bullets
      const buildChangeKey = (c) => {
        const parts = [c.section, c.field];
        if (c.expIndex !== undefined) parts.push(c.expIndex);
        if (c.bulletIndex !== undefined) parts.push(`b${c.bulletIndex}`);
        if (c.itemName) parts.push(c.itemName.toLowerCase().substring(0, 30));
        return parts.join('.');
      };

      const aiChangeKeys = new Set(aiChanges.map(buildChangeKey));
      const additionalChanges = enrichedProgrammaticChanges.filter(c => {
        const key = buildChangeKey(c);
        return !aiChangeKeys.has(key);
      });

      const changesMade = [...aiChanges, ...additionalChanges];
      console.log(`[recompose] Total changes for review: ${changesMade.length} (${aiChanges.length} AI + ${additionalChanges.length} programmatic)`);
