...
export async function executeRecomposition({
  ...
  try {
    ...
    try {
      const changesMade = computeCvDiff(adaptedCv, sourceCv);
      console.log(`[recompose] computeCvDiff detected ${changesMade.length} change(s) for ${generatedFileName}`);
    ...
    // 7. Mettre a jour CvGenerationOffer
