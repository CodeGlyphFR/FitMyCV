  ...
  try {
    ...
    if (!jobOfferId && offer.sourceUrl) {
      try {
        ...
        // Rembourser le crédit
        const refundResult = await refundCreditForOffer(taskId, offer.id, userId, `Extraction failed: ${error.message}`);
    ...
    console.error(`[orchestrator] Single offer task ${taskId} failed:`, error.message);
