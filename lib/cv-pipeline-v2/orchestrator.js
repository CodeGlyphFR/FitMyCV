 ...
 * @param {string} phase - Phase en cours (classify, batches, recompose)
 * @param {string} step - Étape en cours (classify, experiences, projects, extras, skills, summary, recompose)
 * @param {string} status - Statut (running, completed)
 */
function emitProgress(userId, context, phase, step, status) {
...
async function refundCreditForOffer(taskId, offerId, userId, reason = 'Generation failed after max retries') {
  try {
    // Recuperer le cout par offre depuis la tache
    const task = await prisma.cvGenerationTask.findUnique({
      where: { id: taskId },
      select: { creditsDebited: true, totalOffers: true },
    });

    if (!task) {
      console.error(`[orchestrator] Task not found for refund: ${taskId}`);
      return { success: false, error: 'Task not found' };
    }

    const creditPerOffer = Math.floor(task.creditsDebited / task.totalOffers);

    if (creditPerOffer <= 0) {
      console.log(`[orchestrator] No credits to refund for offer ${offerId}`);
      return { success: true, amount: 0 };
    }

    // Utiliser grantCredits avec type 'refund' pour le remboursement
    const result = await grantCredits(userId, creditPerOffer, 'refund', {
    ...
    if (result.success) {
      ...
      await prisma.$transaction([
        ...
        prisma.cvGenerationTask.update({
          where: { id: taskId },
          data: { creditsRefunded: { increment: creditPerOffer } },
        }),
      ]);

      console.log(`[orchestrator] Refunded ${creditPerOffer} credit(s) for offer ${offerId}`);
    }

    return { success: result.success, amount: creditPerOffer };
  } catch (error) {
...
async function runBatchesPhase(offer, sourceCv, classificationResult, jobOffer, targetLanguage, userId, signal, progressContext) {
  ...
  // 2. Skills (apres experiences et projects pour le contexte)
  console.log(`[orchestrator] Running batch: skills`);

  // Émettre progression: skills en cours
  ...
  // Émettre progression: skills terminé
  emitProgress(userId, progressContext, 'batches', 'skills', 'completed');

  // 3. Summary (apres skills et extras pour le contexte complet)
  console.log(`[orchestrator] Running batch: summary`);

  // Émettre progression: summary en cours
  ...
  const summaryResult = await withRetry(
    async () => {
      return await executeBatchSummary({
        ...
        adaptedExperiences: experiencesResult.adaptedExperiences,
        adaptedProjects: projectsResult.adaptedProjects,
        adaptedSkills: skillsResult.adaptedSkills,
  ...
  // Émettre progression: summary terminé
  ...
  return {
    ...
    summary: summaryResult.adaptedSummary,
  };
}
 ...
 */
async function runRecomposePhase(offer, sourceCv, sourceCvFileName, batchResults, jobOffer, targetLanguage, userId, signal, progressContext) {
 ...
 */
async function processOffer(offer, sourceCv, sourceCvFileName, jobOffer, targetLanguage, userId, signal, progressContext, hasAnalyticsConsent = false) {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.error(`[orchestrator] Offer ${offer.id} failed after ${duration}ms:`, error.message);
 ...
 */
export async function startCvGenerationV2(taskId, signal = null) {
  const startTime = Date.now();
  ...
  if (task.status !== 'pending') {
    // Libérer le slot même si la tâche n'est pas en pending
  ...
  for (const offer of task.offers) {
    ...
    if (!jobOffer) {
      console.error(`[orchestrator] Job offer not found: ${offer.jobOfferId}`);
    ...
    const languageInfo = await detectJobOfferLanguage(jobOffer, sourceCvFile.language);
  ...
  const finalStatus = results.completed.length > 0 ? 'completed' : 'failed';
  ...
  // Libérer le slot du type de tâche pour cet utilisateur (succès ou erreur)
  registerTaskTypeEnd(task.userId, 'cv_generation');
}
}
