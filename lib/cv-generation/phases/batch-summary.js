...
/**
 * Extrait les deliverables avec contexte pour le summary
 * Format: "Titre @ Company (description courte) : deliverable1, deliverable2"
 */
function extractDeliverablesWithContext(experiences) {
  ...
  for (const exp of experiences) {
    ...
    const description = exp.description ? exp.description.slice(0, 80) : '';

    const context = company
      ? `${title} @ ${company}${description ? ` (${description}...)` : ''}`
      : `${title}${description ? ` (${description}...)` : ''}`;

    lines.push(`- ${context} : ${deliverables.join(', ')}`);
  }

  return lines.length > 0 ? lines.join('\n') : 'Aucune realisation chiffree disponible';
}
...
export async function executeBatchSummary({
  ...
  try {
    ...
    // Extraire les deliverables avec contexte pour les realisations chiffrees
    const deliverablesWithContext = extractDeliverablesWithContext(adaptedExperiences);
    ...
    const userPrompt = replaceVariables(userPromptTemplate, {
      ...
      deliverables: deliverablesWithContext,
    ...
    const requestOptions = {
      ...
      temperature: 0.3,
      max_completion_tokens: 800, // Limite de securite - sortie attendue ~300 tokens
    };
