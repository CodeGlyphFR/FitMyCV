...
async function processOffer(offer, sourceCv, sourceCvFileName, jobOffer, jobOfferId, targetLanguage, userId, signal, progressContext, hasAnalyticsConsent = false) {
  ...
  try {
    ...
    // Télémétrie: CV_GENERATION_COMPLETED (Story 5.3)
    ...
    // Télémétrie: CV_GENERATION_FAILED (Story 5.3)
    if (hasAnalyticsConsent) {
...
/**
 * Démarre la génération CV pour une tâche
 *
 * @param {string} taskId - ID de la CvGenerationTask
 * @returns {Promise<Object>}
 */
export async function startCvGeneration(taskId) {
  const startTime = Date.now();
  ...
  console.log(`[orchestrator] Starting CV generation for task ${taskId}`);
  ...
  try {
    ...
    // Télémétrie: CV_GENERATION_STARTED (Story 5.3)
