 ...
 * @param {string} settingName - Nom du setting (ex: "model_analysis_rapid", "model_match_score")
...
/**
 * Récupère le modèle pour un niveau d'analyse donné
 * @param {string} analysisLevel - Niveau d'analyse ('rapid', 'medium', 'deep')
 * @returns {Promise<string>} - Nom du modèle
 * @throws {Error} Si le niveau d'analyse est invalide ou le setting n'existe pas
 */
export async function getAiModelForAnalysisLevel(analysisLevel) {
  const settingName = `model_analysis_${analysisLevel}`;
  return await getAiModelSetting(settingName);
}
