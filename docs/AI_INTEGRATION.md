...
- [Rate Limiting & Tokens](#rate-limiting--tokens)
...
Chaque utilisateur dispose d'un nombre limité de **tokens** pour le match score :

- **Par défaut** : 5 tokens (configurable via `default_token_limit` setting)
- **Consommation** : 1 token par calcul de match score
- **Reset** : À 24h d'inactivité (pas de token utilisé pendant 24h)
...
// Vérifier si reset nécessaire (24h)
const now = new Date();
const lastUsage = user.tokenLastUsage;
const hoursSinceLastUsage = (now - lastUsage) / (1000 * 60 * 60);

if (hoursSinceLastUsage >= 24) {
  // Reset à la limite par défaut
  await prisma.user.update({
    where: { id: userId },
    data: {
      matchScoreRefreshCount: defaultTokenLimit,
      tokenLastUsage: now,
    }
  });
} else if (user.matchScoreRefreshCount > 0) {
  // Décrémenter
  await prisma.user.update({
    where: { id: userId },
    data: {
...
- **TokenCounter** : Affiche tokens restants + barre de progression
- **Couleurs** :
  - Vert (5-4 tokens)
  - Jaune (3-2 tokens)
  - Rouge (1-0 tokens)
