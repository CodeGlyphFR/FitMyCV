 ...
 *
 * Usage: node scripts/sync-stripe.mjs [--quiet]
 */
...
if (!process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY === 'sk_test_TODO') {
  if (!quiet) console.error('❌ STRIPE_SECRET_KEY non configuré dans .env');
  process.exit(1);
}

if (!process.env.DATABASE_URL) {
  if (!quiet) console.error('❌ DATABASE_URL non configuré dans .env');
  process.exit(1);
}
